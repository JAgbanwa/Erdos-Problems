...
## Status / TODO before submitting upstream
* `conic_caratheodory` is free of `grind`, `simp +decide`, and `simp_all`, and compiles at
  the **default heartbeat budget** (no `set_option maxHeartbeats`) — fully idiomatic. ✓
* `ConvexCone` / `PointedCone` API integration done: `span_isClosed_of_finite`,
  `isSimplicial_isClosed`. ✓
* Remaining (optional): generalize the scalar field beyond `ℝ`; Mathlib Zulip discussion
  on placement/naming (see `ZULIP_TOPIC_DRAFT.md`).
-/
import Mathlib
...
lemma conic_caratheodory [Fintype κ] [DecidableEq κ]
  ...
  obtain ⟨s, hs⟩ : ∃ s : Finset κ, (∃ d : κ → ℝ, (∀ k, 0 ≤ d k) ∧ (∀ k ∉ s, d k = 0) ∧ (∑ k ∈ s, d k • v k = ∑ k, c k • v k)) ∧ ∀ t : Finset κ, (∃ d : κ → ℝ, (∀ k, 0 ≤ d k) ∧ (∀ k ∉ t, d k = 0) ∧ (∑ k ∈ t, d k • v k = ∑ k, c k • v k)) → s.card ≤ t.card := by
    ...
    · exact ⟨ Finset.univ, ⟨ c, hc, fun k hk => False.elim <| hk <| Finset.mem_univ _, by simp ⟩ ⟩;
  ...
  · obtain ⟨d, hd_nonneg, hd_zero, hd_sum⟩ := hs.left
    obtain ⟨a, ha_nonzero, ha_support, ha_sum⟩ : ∃ a : κ → ℝ, (∃ k ∈ s, a k ≠ 0) ∧ (∀ k ∉ s, a k = 0) ∧ (∑ k ∈ s, a k • v k = 0) ∧ (∃ k ∈ s, a k > 0) := by
      obtain ⟨a, ha_nonzero, ha_sum⟩ : ∃ a : κ → ℝ, (∃ k ∈ s, a k ≠ 0) ∧ (∀ k ∉ s, a k = 0) ∧ (∑ k ∈ s, a k • v k = 0) := by
        ...
        refine ⟨fun k => if k ∈ t then g k else 0, ⟨i, ht hi, by simp only [if_pos hi]; exact hi'⟩,
          fun k hk => if_neg (fun h => hk (ht h)), ?_⟩
    ...
    obtain ⟨θ, hθ_pos, hθ_min⟩ : ∃ θ > 0, (∀ k ∈ s, d k - θ * a k ≥ 0) ∧ (∃ k ∈ s, d k - θ * a k = 0) := by
      obtain ⟨k₀, hk₀⟩ : ∃ k₀ ∈ s, a k₀ > 0 ∧ ∀ k ∈ s, a k > 0 → d k / a k ≥ d k₀ / a k₀ := by
        ...
        exact ⟨k₀, hk₀s, hk₀pos, fun k hk hk' => hk₀min k (Finset.mem_filter.mpr ⟨hk, hk'⟩)⟩
      refine' ⟨ d k₀ / a k₀, div_pos ( lt_of_le_of_ne ( hd_nonneg k₀ ) ( Ne.symm _ ) ) hk₀.2.1, _, k₀, hk₀.1, _ ⟩;
      · -- d k₀ ≠ 0: else (s.erase k₀, d) is a strictly smaller feasible support, contradicting minimality
        intro h
        ...
        have hval : ∑ k ∈ s.erase k₀, d k • v k = ∑ k, c k • v k := by
          rw [Finset.sum_erase s hk₀v]; exact hd_sum
      ...
      · intro k hk
        by_cases hk' : a k > 0
        · have hle : d k₀ / a k₀ * a k ≤ d k := (le_div_iff₀ hk').1 (hk₀.2.2 k hk hk')
          linarith
        · have h2 : d k₀ / a k₀ * a k ≤ 0 :=
            mul_nonpos_of_nonneg_of_nonpos (div_nonneg (hd_nonneg _) hk₀.2.1.le) (le_of_not_gt hk')
    ...
    have h_t_support : ∀ k, 0 ≤ d k - θ * a k := by
      exact fun k => if hk : k ∈ s then hθ_min.1 k hk else by simp [ hd_zero k hk, ha_support k hk ] ;
    have h_t_zero : ∀ k ∉ t, d k - θ * a k = 0 := by
      intro k hk
      by_cases hks : k ∈ s
      · by_contra hP
        exact hk (by rw [ht_def]; exact Finset.mem_filter.mpr ⟨hks, hP⟩)
      · rw [hd_zero k hks, ha_support k hks]; ring
...
`Submodule {c : ℝ // 0 ≤ c} E`; `PointedCone.span ℝ s` is its span. The bridge below
...
private lemma span_eq_engine {s : Set E} [Fintype s] :
    (PointedCone.span ℝ s : Set E)
  ...
  · -- engine set ⊆ span
    ...
    have hk : (k : E) ∈ PointedCone.span ℝ s := PointedCone.subset_span k.2
...
theorem span_isClosed_of_finite {s : Set E} (hs : s.Finite) :
    IsClosed (PointedCone.span ℝ s : Set E) := by
  classical
...
theorem isSimplicial_isClosed {C : PointedCone ℝ E} (hC : C.IsSimplicial) :
  ...
  rw [← hspan]
  exact span_isClosed_of_finite hs
