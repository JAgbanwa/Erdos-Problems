...
\textbf{Preprint:} version 1.2. This manuscript and its accompanying formal artifact form one self-contained public package.\\
...
\textbf{Keywords:} chordal graph; fractional triangle cover; symmetrization; clone class; complete-split graph; Erdős problem.
...
\paragraph{Corollary 1.2}\label{corollary-1.2}
...
\paragraph{\texorpdfstring{Corollary 1.2\(^{\prime}\) (sharpened asymptotic)}{Corollary 1.2\^{}\{\textbackslash prime\} (sharpened asymptotic)}}\label{corollary-1.2prime-sharpened-asymptotic}
...
\texttt{PaperII.phiTau\_max\_sandwich} (axiom footprint \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}).

\textbf{Formalization status.} The delivered Paper II Lean v1.2 freeze uses Lean 4 / Mathlib v4.28.0 {[}5,6{]}. Its recorded main build completed 8,061 jobs, and the explicit extremizer/copy-defect supplement completed 8,032 jobs. The main declaration is named \texttt{PaperII.theorem\_1\_2}; it corresponds to Theorem 1.1 of this manuscript and includes the accompanying attainment statement. The same freeze includes the complete-split argmax, level sets, copy defects, and \texttt{PaperII.AsymptoticCorollaries}. Its theorem-level axiom gate reports no project-level mathematical axiom. The formal development derives the chordal structure used in the proof from the standard predicate \texttt{IsChordal}; no clique-tree or chordal-structure axiom is added. Section 8 records the exact freeze perimeter.
...
\paragraph{Remark (chordality is essential)}\label{remark-chordality-is-essential}
...
\paragraph{Remark (coherence with Paper I's bound)}\label{remark-coherence-with-paper-is-bound}
...
\texttt{PaperII.phiTau\_max\_le\_paperI\_bound} (axiom footprint \texttt{propext}, \texttt{Quot.sound}).
...
\subsection{2. Definitions and chordal preliminaries}\label{definitions-and-chordal-preliminaries}
...
\textbf{Relation to the series (not used in the proof).} By finite linear-programming duality the fractional triangle-cover and fractional triangle-packing optima coincide, \(\nu_3^*(G)=\tau_3^*(G)\). Hence
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The complete-split terminal family.}]{figures/fig1_complete_split_en.png}}
\caption{The complete-split terminal family.}
\end{figure}

\textbf{Figure 1.} The complete-split graph \(S_{p,q}=K_p\vee\overline{K_q}\): the clique \(K_p\) is joined completely to the independent set of order \(q\), and every triangle uses at least one clique edge. Admissible copies reduce the extremal problem to this family.
...
A vertex \(v\) is \textbf{simplicial} if \(N_G(v)\) is a clique. A clone class is simplicial if its common open neighborhood is a clique.
...
\item
  Adding a new vertex whose neighborhood is a clique preserves chordality.
\item
  Every nonempty chordal graph has a simplicial vertex, and every noncomplete connected chordal graph has two nonadjacent simplicial vertices.
\item
  Every connected chordal graph has a clique tree: its nodes are the maximal cliques, and the maximal cliques containing any fixed vertex induce a connected subtree.
\end{enumerate}
...
\subsection{3. The vertex-copy inequality}\label{the-vertex-copy-inequality}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The vertex-copy exchange.}]{figures/fig2_copy_exchange_en.png}}
\caption{The vertex-copy exchange.}
\end{figure}

\textbf{Figure 2.} The exchange step (Lemma 3.1). Copying \(v\) toward \(u\) makes \(v\) a clone of \(u\); the two opposite copies satisfy \(\Phi_\tau(G_{v\to u})+\Phi_\tau(G_{u\to v})\ge 2\Phi_\tau(G)\).
...
\paragraph{Corollary 3.2}\label{corollary-3.2}
...
\paragraph{Remark (alternative packing-side proof)}\label{remark-alternative-packing-side-proof}
...
\paragraph{Definition (copy defect)}\label{definition-copy-defect}
...
\end{aligned}
\]
Then \(\Gamma_{\Phi_\tau}\ge\Delta_{\Phi_\tau}/2\), formalized as \texttt{copyDefect\_nonneg} and \texttt{copyGamma\_ge\_half\_copyDefect}. The quantity \(\Delta_{\Phi_\tau}\) is the resource that any future stability statement must control; it is recorded here as a quantitative handle, not as a stability theorem.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{4. Copying whole clone classes}\label{copying-whole-clone-classes}
...
\paragraph{Lemma 4.1}\label{lemma-4.1}
...
\paragraph{Lemma 4.2}\label{lemma-4.2}
...
\paragraph{Lemma 4.3}\label{lemma-4.3}
...
\subsection{5. Terminal chordal graphs and symmetrization}\label{terminal-chordal-graphs-and-symmetrization}
...
\paragraph{Theorem 5.3}\label{theorem-5.3}
...
\subsection{6. The complete-split cover program}\label{the-complete-split-cover-program}
...
\paragraph{Lemma 6.1}\label{lemma-6.1}
...
\paragraph{Proposition 6.2}\label{proposition-6.2}
...
\paragraph{Corollary 6.3}\label{corollary-6.3}
...
\textbf{Table 1. The complete-split cover functional.} With \(P=\binom p2\), \(n=p+q\); the two non-degenerate branches agree at \(q=p-1\).
...
\subsection{7. Exact integer maximization}\label{exact-integer-maximization}

\paragraph{Proposition 7.1}\label{proposition-7.1}
...
\paragraph{Proof}\label{proof-11}
...
\paragraph{Proof of Theorem 1.1}\label{proof-of-theorem-1.1}
...
\paragraph{Corollary 7.2 (level sets of the cover functional)}\label{corollary-7.2-level-sets-of-the-cover-functional}
...
\]
with \(\delta\) measured from the continuous maximum \(M^2/24\). This is an exact equivalence, obtained from identity (7.1) by monotonicity of the square, and is formalized as \texttt{level\_set\_iff}. In particular \(\Phi_\tau\) is not flat in any direction of \(p\), and its level sets are intervals of radius \(\sqrt{2\delta/3}\); within the complete-split family this settles the second half of the stability question raised in Paper III, §12.4.

\paragraph{Corollary 7.3 (residue-class formula)}\label{corollary-7.3-residue-class-formula}
...
Thus the extremal value is one of these two integers according to the residue of \(n\) modulo \(3\). This makes explicit the odd-square-modulo-\(24\) fact used in the case analysis of Proposition 7.1. The same residue class \(n\equiv1\pmod3\) is exactly the two-value tie case recorded by \texttt{Fsat\_argmax\_tie}. The residue fact is formalized as
\texttt{PaperII.odd\_sq\_emod\_24} and the closed form as \texttt{PaperII.phiTau\_max\_closed} (axiom footprint
\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}).

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsubsection{Numerical illustration and small cases}\label{numerical-illustration-and-small-cases}

\textbf{Table 2. Small cases of the extremal value.} Optimal \(p\) is an integer nearest to \((2n+1)/6\); the extremizer is \(S_{p,\,n-p}\).
...
\begin{figure}
\centering
\includegraphics[width=0.78\linewidth,height=\textheight,keepaspectratio,alt={\textbackslash Phi\_\textbackslash tau(S\_\{p,q\}) as a function of the clique size p for n=12.}]{figures/fig3_phitau_profile_en.png}
\caption{\(\Phi_\tau(S_{p,q})\) as a function of the clique size \(p\) for \(n=12\).}
\end{figure}

\textbf{Figure 3.} The two branches of \(\Phi_\tau(S_{p,q})\) for \(n=12\), together with the floor \(\lfloor(2n+1)^2/24\rfloor=26\) and the continuous peak \((2n+1)^2/24\approx26.04\). The integer maximizer \(p=4\) attains the floor.
...
\textbf{Table 3. Extended exact values.} The table gives the exact maximum and one optimal complete-split graph for selected orders.
...
\subsection{8. Reproducibility, formal verification, and scope}\label{reproducibility-formal-verification-and-scope}
...
\subsubsection{Formal verification in Lean}\label{formal-verification-in-lean}

The consolidated snapshot at \texttt{PAPER\_II/05\_formalization/lean\_v1.2\_freeze} uses Lean 4 / Mathlib v4.28.0 {[}5,6{]}. The recorded main build completed 8,061 jobs with zero errors; an explicit supplement covering \texttt{PaperII.Extremizer} and \texttt{PaperII.CopyDefect} completed 8,032 jobs with zero errors. The corresponding main declaration is
...
The formalization uses the standard chordal predicate \texttt{IsChordal}. In the development, this predicate says that every cycle of length at least four has a chord. The deficit functional is formalized as

\[
\texttt{phiTau G = (|E(G)| : \ensuremath{\mathbb{R}}) - 2 · tau3star G},
\]

where \texttt{tau3star} is the fractional triangle-cover number of the finite cover LP. The complete-split family is formalized as \texttt{completeSplit\ p\ q}, and the extremal value is the integer \(\lfloor(2n+1)^2/24\rfloor\).

No chordal-structure axiom is added. The formalization derives from \texttt{IsChordal} the hereditary property for induced subgraphs, preservation under adding a vertex with clique neighborhood, and the existence of simplicial vertices. The written proof of Lemma 5.1 uses clique trees as standard textbook infrastructure; the Lean proof uses a clique-tree-free terminal-characterization argument. Thus the formal proof does not assume a clique-tree or running-intersection axiom.
...
These are Lean's standard classical axioms. The report contains no \texttt{sorryAx} and no project-specific axiom. The frozen source archive is \texttt{PAPER\_II\_lean\_v1.2\_freeze.zip}, with SHA-256 \texttt{ee2d05cc40d943ca92f8f7bf3e5dd83c2692518ddea5e2ca4f7686ccb1ac3895}. The main and supplement logs are stored as \texttt{gate\_logs/BUILD\_LOG.txt} and \texttt{gate\_logs/BUILD\_SUPPLEMENT\_EXTREMIZER\_COPYDEFECT.txt} within \texttt{lean\_v1.2\_freeze}. The chordal-graph contribution modules shipped in the same archive are candidates for upstreaming to Mathlib and are not additional proof assumptions of this paper. The frozen archive passed independent clean-room reproduction; the external audit recorded a successful 8,063-job build and the expected 16 axiom surfaces.

\textbf{Table 4. Frozen formalization perimeter (Lean v1.2).} The delivered freeze contains the main theorem and the manuscript declarations listed below; independent clean-room reproduction returned PASS.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{PaperII.theorem\_1\_2} & \(\max_{\text{chordal},\,|V|=n}\Phi_\tau=\lfloor(2n+1)^2/24\rfloor\), attained by a complete-split graph & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} \\
\texttt{Fsat\_argmax\_unique}, \texttt{Fsat\_argmax\_tie} & uniqueness of the maximizing integer \(p\) within the complete-split family, and the two-value tie when \(n\equiv1\pmod3\) & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} \\
\texttt{level\_set\_iff} (Cor. 7.2) & exact level-set equivalence from the continuous maximum & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} \\
\texttt{copyDefect\_nonneg}, \texttt{copyGamma\_ge\_half\_copyDefect} & copy defect \(\Delta_{\Phi_\tau}\ge0\) and \(\Gamma_{\Phi_\tau}\ge\Delta_{\Phi_\tau}/2\) & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} \\
\end{longtable}
}

\textbf{Table 5. Arithmetic surface included in the Lean v1.2 freeze.} These declarations are consequences of the frozen extremal value and are included in the delivered archive.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{phiTau\_max\_sandwich} (Cor. 1.2\(^{\prime}\)) & for every \(n\in\mathbb Z\), \(4n^2+4n-23<24\lfloor(2n+1)^2/24\rfloor\le4n^2+4n+1\) & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} \\
\texttt{odd\_sq\_emod\_24}, \texttt{phiTau\_max\_closed} (Cor. 7.3) & \((2n+1)^2\equiv1\) or \(9\pmod{24}\), and the resulting residue-class formula for the extremum & \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound} \\
\texttt{phiTau\_max\_le\_paperI\_bound} & \(\lfloor(2n+1)^2/24\rfloor\le n^2/6+n/2\) for \(n\ge1\) (comparison with Paper I) & \texttt{propext}, \texttt{Quot.sound} \\
\end{longtable}
}

The four declarations in Table 5 are collected in \texttt{PaperII/AsymptoticCorollaries.lean}, which builds against the same toolchain (Lean 4 / Mathlib v4.28.0) and carries only Lean's standard classical axioms, with no \texttt{sorryAx} and no project-specific axiom. The per-declaration \texttt{\#print\ axioms} reports are archived in \texttt{PaperII/AxiomCheckCorollaries.lean}. The module adds no hypothesis to and does not modify the frozen main development; each declaration is derived from the closed value by \texttt{ring}, a finite residue split, and \texttt{omega}.

\subsubsection{Independent computational verification}\label{independent-computational-verification}
