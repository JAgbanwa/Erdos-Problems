...
**Author manuscript:** v1.1.8 near-final editorial candidate for external adversarial AI audit. This candidate supersedes v1.1.7. The v1.1.8 changes are editorial only: they close the remaining domain clarification in Corollary 1.2′ while preserving the separation between the frozen main development and the post-freeze arithmetic module. Version v1.1.6 added three elementary arithmetic consequences of the frozen extremal value \(\lfloor(2n+1)^2/24\rfloor\): (i) a sharpened form of Corollary 1.2 with an explicit bounded remainder (Corollary 1.2′); (ii) an exact residue-class formula for the extremal value (Corollary 7.3); and (iii) a numerical comparison with Paper I's split-graph bound (Remark, Section 1). No pre-existing theorem statement, definition, constant, displayed identity, or proof in Sections 2–7 was changed in v1.1.6, and no mathematical content is changed in v1.1.7. Version v1.1 had added the uniqueness/tie characterization of the maximizing complete-split parameter, Corollary 7.2 on level sets of the cover functional, the copy-defect quantities \(\Delta_{\Phi_\tau}\) and \(\Gamma_{\Phi_\tau}\), and two explanatory remarks. The existing arguments in Sections 2–7 and the symmetrization proof are retained. A separate post-freeze Lean module, `PaperII.AsymptoticCorollaries`, records `phiTau_max_sandwich`, `odd_sq_emod_24`, `phiTau_max_closed`, and `phiTau_max_le_paperI_bound`; the frozen declarations `Fsat_argmax_unique`, `Fsat_argmax_tie`, `level_set_iff`, `copyDefect_nonneg`, `copyGamma_ge_half_copyDefect`, and `PaperII.theorem_1_2` are unchanged.  
**Date:** August 1, 2026  
**Status:** near-final editorial candidate for external adversarial AI audit. The Paper II Lean v1.0.1 freeze builds successfully against Lean 4 / Mathlib v4.28.0 with no project-level mathematical axioms. The freeze contains the exact extremal theorem and the v1.1 declarations listed in Table 4. The v1.1.6 arithmetic consequences are checked in a separate post-freeze module listed in Table 5; they do not modify the v1.0.1 archive. The project also reports completion of the associated computational regression tests without discrepancy. Archive details and linter information are collected in Section 8. Independent reproduction of the frozen archive, external peer review, and the prior-art/novelty assessment remain separate release gates.
...
#### Corollary 1.2′ (sharpened asymptotic)
...
**Formalization status.** The Paper II Lean v1.0.1 freeze builds successfully against Lean 4 / Mathlib v4.28.0 [5,6] with no project-level mathematical axioms. For historical compatibility with the Lean ledger, the main declaration is named `PaperII.theorem_1_2`, although it corresponds to Theorem 1.1 of the present manuscript and includes the accompanying attainment statement. The freeze also includes the v1.1 declarations on the complete-split argmax, level sets, and copy defects. The v1.1.6 arithmetic consequences are verified separately in `PaperII/AsymptoticCorollaries.lean` and are not part of the v1.0.1 freeze. The formal development derives the chordal structure used in the proof from the standard predicate `IsChordal`; no clique-tree or chordal-structure axiom is added. Section 8 records the freeze perimeter.
...
\[
\Delta_{\Phi_\tau}(u,v)=\Phi_\tau(G_{v\to u})+\Phi_\tau(G_{u\to v})-2\Phi_\tau(G)\ \ge\ 0\quad(\text{Lemma 3.1}),\qquad \Gamma_{\Phi_\tau}(u,v)=\max\{\Phi_\tau(G_{v\to u}),\Phi_\tau(G_{u\to v})\}-\Phi_\tau(G).
\]
...
The frozen snapshot at `PAPER_II/05_formalization/lean_v1.0.1_freeze` builds successfully against Lean 4 / Mathlib v4.28.0 [5,6]. The recorded build completed 8,059 jobs with zero errors. The corresponding main declaration is
...
These are Lean's standard classical axioms. The report contains no `sorryAx` and no project-specific axiom. The frozen source archive is `PAPER_II_LEAN_v1.0.1_FREEZE_sources.zip`, with SHA-256 `dacccc06ff265a2a0da55ed3aac1093486d07806cd0435831738ebca94a4077f`. The full build log is stored under `PAPER_II/05_formalization/lean_v1.0.1_freeze/gate_logs/lake_build_full.log`. The separately maintained chordal-graph contribution modules are candidates for upstreaming to Mathlib and are not additional proof assumptions of this paper. Independent reproduction of the frozen archive remains pending.

**Table 4. Frozen formalization perimeter (Lean v1.0.1).** The freeze contains the main theorem and the v1.1 manuscript declarations listed below; independent reproduction remains pending.
...
**Table 5. Post-freeze arithmetic module (v1.1.6).** These declarations are consequences of the frozen extremal value and are not part of the v1.0.1 archive.

| Node | Statement | Axiom footprint |
|---|---|---|
| `phiTau_max_sandwich` (Cor. 1.2′) | for every \(n\in\mathbb Z\), \(4n^2+4n-23<24\lfloor(2n+1)^2/24\rfloor\le4n^2+4n+1\) | `propext`, `Classical.choice`, `Quot.sound` |
...
[4] J. P. Traverso Gianini, “Affine profile reduction for fractional triangle packings in split graphs” (Paper I in the series), preprint v1.1, July 2026.
