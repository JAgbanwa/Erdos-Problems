...
def main() -> int:
    ...
    metadata = {
        "en": pdf_metadata(FILES["pdf_en"]),
        "es": pdf_metadata(FILES["pdf_es"]),
    }
    ...
    for lang in ("en", "es"):
        ...
        tex_body = tex.split("\\maketitle", 1)[1]
        checks.extend(
            [
                {
                    "check": f"single-markdown-author-block:{lang}",
                    "pass": md.count("**Juan Pablo Traverso Gianini**") == 1,
                    "detail": "Markdown contains exactly one GitHub-visible author block",
                },
                {
                    "check": f"no-visible-author-duplication-after-maketitle:{lang}",
                    "pass": all(
                        token not in tex_body
                        for token in (
                    ...
                    "detail": "the generated body does not repeat the template author block",
                },
                {
                    "check": f"single-rendered-author-name:{lang}",
                    "pass": pdf.count("Juan Pablo Traverso Gianini") == 1,
                    "detail": "extracted PDF text contains the visible author name exactly once",
                },
                {
                    "check": f"pdf-author-metadata:{lang}",
                    "pass": "Author:          Juan Pablo Traverso Gianini" in metadata[lang]
                    and "AUTHORBLOCK" not in metadata[lang],
                    "detail": "PDF metadata names the author and contains no placeholder",
                },
                {
                    "check": f"public-preprint-front-matter:{lang}",
                    "pass": (
                        ("**Preprint:** version 1.5" in md if lang == "en" else "**Preprint:** versión 1.5" in md)
                        and "Preprint draft" not in md
                        and "Borrador de preprint" not in md
                    ),
                    "detail": "the public manuscript identifies preprint v1.5 without draft wording",
                },
            ]
        )
    ...
    output = ROOT / "03_reproducibility" / "MANUSCRIPT_CONSISTENCY_RESULTS_v1.5.json"
