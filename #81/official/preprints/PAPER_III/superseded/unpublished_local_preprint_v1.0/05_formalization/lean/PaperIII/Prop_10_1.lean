...
/-- **Prop-10.1, low-corridor part**: for `p ≥ 36` and `0 ≤ s`, `s² ≤ 36p`,
`Φ(G) ≤ n²/6 + 2n` (LEDGER Prop-10.1; from Corollary 5.3, no axioms). -/
theorem Prop_10_1_low (G : SplitGraph) (hp : 36 ≤ G.p)
    (hs0 : 0 ≤ G.s) (hs : (G.s : ℝ) ^ 2 ≤ 36 * (G.p : ℝ)) :
    ((G.Phi : ℤ) : ℝ) ≤ (G.n : ℝ) ^ 2 / 6 + 2 * (G.n : ℝ) := by
  -- relations: (n:ℝ) = p + q, (s:ℝ) = 2p − q, p ≥ 36, s ≥ 0, s² ≤ 36p
  have hn : (G.n : ℝ) = (G.p : ℝ) + (G.q : ℝ) := by
    rw [SplitGraph.n]; push_cast; ring
  have hsr : (G.s : ℝ) = 2 * (G.p : ℝ) - (G.q : ℝ) := by
    rw [SplitGraph.s]; push_cast; ring
  have hs0' : (0 : ℝ) ≤ (G.s : ℝ) := by exact_mod_cast hs0
  have hp' : (36 : ℝ) ≤ (G.p : ℝ) := by exact_mod_cast hp
  -- corridor gives s ≤ p (from s² ≤ 36p ≤ p²) hence q = 2p − s ≥ p ≥ r_p, q ≥ 1
  have hsp : (G.s : ℝ) ≤ (G.p : ℝ) := by nlinarith [hs, hs0', hp']
  have hqp : G.p ≤ G.q := by
  ...
  -- final algebra: p/2 + (s²−6s+3)/12 ≤ 2n = 2(p+q), using s²≤36p, p≥36, s≥0
  rw [hn] at h53 ⊢
  rw [hsr] at h53 hs hsp
  nlinarith [h53, hs, hs0', hp', hsp, sq_nonneg ((G.s : ℝ))]
