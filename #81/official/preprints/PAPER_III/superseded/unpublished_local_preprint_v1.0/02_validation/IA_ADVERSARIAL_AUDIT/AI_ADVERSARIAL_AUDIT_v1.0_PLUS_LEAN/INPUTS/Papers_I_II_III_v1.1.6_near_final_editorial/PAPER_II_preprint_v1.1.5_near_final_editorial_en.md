...
**Author manuscript:** v1.1.5 near-final editorial candidate for external adversarial AI audit. This candidate supersedes v1.1.4. All v1.1.5 changes are editorial and concern verification status, release-layer organization, and terminology; theorem statements, definitions, constants, displayed identities, and proofs are unchanged relative to v1.1.4. Version v1.1 adds the uniqueness/tie characterization of the maximizing complete-split parameter, Corollary 7.2 on level sets of the cover functional, the copy-defect quantities \(\Delta_{\Phi_\tau}\) and \(\Gamma_{\Phi_\tau}\), and two explanatory remarks. The existing arguments in Sections 2–7 and the symmetrization proof are retained. The Lean development adds `Fsat_argmax_unique`, `Fsat_argmax_tie`, `level_set_iff`, `copyDefect_nonneg`, and `copyGamma_ge_half_copyDefect`; the declaration `PaperII.theorem_1_2` is retained.  
**Date:** July 28, 2026  
**Status:** near-final editorial candidate for external adversarial AI audit. The Paper II Lean v1.0.1 freeze builds successfully against Lean 4 / Mathlib v4.28.0 with no project-level mathematical axioms. The freeze contains the exact extremal theorem and the v1.1 declarations listed in Section 8. The project also reports completion of the associated computational regression tests without discrepancy. Archive details and linter information are collected in Section 8. Independent reproduction of the frozen archive, external peer review, and the prior-art/novelty assessment remain separate release gates.
