...
\begin{figure}[H]
\centering
\includegraphics[width=0.78\linewidth]{paperIII_figures_v0.9.12/fig1_complete_split_extremizer_en.pdf}
...
\textbf{Table 1. The three proof regimes.} The near-extremal corridor is elementary and effective (Proposition 10.5); the other two regimes each invoke exactly one external asymptotic input.
...
\bottomrule\noalign{}
\endlastfoot
Sparse independent side & \(\alpha\to 0\) & dense clique residual admits an exact triangle decomposition & Theorem 2.3 (Dross--BKLO) & \S{}8 \\
Bulk & \(\alpha\) bounded away from \(0\) and \(2\) & quadratic fractional margin (common-profile LP + fractional cloning) & Theorem 2.1 (Haxell--Rödl/Yuster) & \S{}3--4, \S{}9.1 \\
Near-extremal corridor & \(\alpha\to 2\), \(q=2p-s\) & averaged factorization, double-factor polarization, shifted-center gain completion & none (elementary/effective) & \S{}5--7, \S{}9.2--9.3, \S{}10.5 \\
\end{longtable}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.86\linewidth]{paperIII_figures_v0.9.12/fig2_alpha_regimes_en.pdf}
...
\subsubsection{Theorem 2.1 --- Haxell--Rödl/Yuster {[}7,9{]}}\label{theorem-2.1-haxellruxf6dlyuster-79}
...
Papers I and II of the series have frozen Lean 4 / Mathlib releases. The present paper is being prepared under the same release protocol. The author-supplied report records a Mathlib v4.28.0 build comprising 8,057 jobs, zero errors, and zero \texttt{sorry}; the public commit and an independent reproduction remain pending.
...
\tightlist
\item
  \textbf{Layer X} contains exactly the two external asymptotic inputs, Theorems 2.1 and 2.3, retained as named axioms \texttt{AX1} and \texttt{AX2} with their hypotheses stated explicitly. In the formalization \texttt{AX1} is stated on the cover side (\(\tau_3^*-\nu_3=o(n^2)\)); the proved weak-duality inequality \(\nu_3^*\le\tau_3^*\) is sufficient for the bulk deduction used here. No additional strong-duality assumption is required for that step.
\item
  \textbf{Layer E} contains the finite mathematical core recorded by the supplied report as proved from first principles and machine-checked: Theorems 3.1 and 4.2; Lemmas 4.1--7.1; the divisibility correction of Appendix B; the list edge coloring development of Appendix D; the quantitative chains of Sections 9.3 and 10.5; Proposition 10.5; and the final assembly of Theorem 1.1. Two reusable classical results absent from Mathlib --- Dirac's Hamiltonicity theorem and a near-perfect matching from minimum degree --- are also proved from scratch and are axiom-clean.
\end{itemize}

The supplied report records the following axiom footprints: Theorem 1.1 and Corollary 1.2 carry \texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}, \texttt{AX1}, and \texttt{AX2}; the bulk node carries \texttt{AX1} in addition to the standard triple; and Proposition 10.5 together with the elementary lemmas and the divisibility correction carry only the standard triple. The same report lists both \texttt{AX1} and \texttt{AX2} for the sparse-regime node, although the manuscript proof in Section 8 visibly invokes only Theorem 2.3 (\texttt{AX2}) among the two named asymptotic inputs. Whether \texttt{AX1} is a genuine transitive dependency or an avoidable import remains a release-gate item to be reconciled against the frozen axiom report. The supplied report records no \texttt{sorryAx}, \texttt{admit}, or \texttt{native\_decide}, and identifies \texttt{AX1} and \texttt{AX2} as the only project axiom declarations.

\textbf{Table 2. Reported formalization perimeter.} Layer X contains the two external inputs retained as named axioms; Layer E is the finite core recorded as machine-checked relative to those axioms. ``Lean report: proved'' refers to the author-supplied build and axiom report; independent reproduction and human peer review remain pending.
...
X & \texttt{AX2} & Dross--BKLO: triangle-divisible, \(\delta\ge(0.9+\varepsilon)n\) \(\Rightarrow\) triangle decomposition & Named axiom (external) \\
...
E & Bulk node (\S{}9.1) & fractional-to-integral in the bulk & Lean report: proved, modulo \texttt{AX1} \\
E & Sparse node (\S{}8) & dense residual decomposition & Lean report: proved, reported modulo \texttt{AX1}, \texttt{AX2}; manuscript proof visibly uses \texttt{AX2}; dependency reconciliation pending \\
E & \textbf{Thm 1.1} & main theorem (assembly) & \textbf{Lean report: proved, modulo \texttt{AX1}, \texttt{AX2}} \\
\end{longtable}
...
\subsubsection{Corollary 12.3 --- Effective corridor algorithm}\label{corollary-12.3-effective-corridor-algorithm}
...
\NormalTok{    PAPER\_III\_split\_lineal\_v0.9.12\_editorial\_review\_en.md}
...
\subsubsection{Lemma D.2 --- Gale--Shapley {[}11{]}}\label{lemma-d.2-galeshapley-11}
