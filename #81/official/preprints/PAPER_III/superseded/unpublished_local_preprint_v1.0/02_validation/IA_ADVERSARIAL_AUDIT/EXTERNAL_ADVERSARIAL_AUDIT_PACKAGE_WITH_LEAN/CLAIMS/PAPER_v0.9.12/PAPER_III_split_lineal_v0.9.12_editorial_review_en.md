...
**Author manuscript:** v0.9.12 editorial review candidate. The mathematical statements, hypotheses, constants, displayed identities, and proofs in Sections 2–9 and Appendices A–D are unchanged from v0.9.7, apart from cross-reference labels updated in v0.9.10. Version 1.0 is reserved for the first public preprint.  
**Date:** July 25, 2026  
**Status:** editorial preprint candidate. The author-supplied Lean 4 / Mathlib v4.28.0 build and axiom report records a zero-`sorry` compile for the proved core, with no `admit`, `native_decide`, or `unsafe`. Its trusted project layer consists of exactly the two external asymptotic inputs of Section 2.4, retained as the named axioms `AX1` (Haxell--Rödl/Yuster) and `AX2` (Dross + Barber--Kühn--Lo--Osthus). The reported axiom footprint of Theorem 1.1 is `propext`, `Classical.choice`, `Quot.sound`, `AX1`, and `AX2`, and the report records no other project axiom declaration. The public release commit, an independent reproduction of the build, human peer review, and the prior-art assessment remain pending; final verification wording is tied to the frozen release. See Section 11.6 for the full perimeter.
...
All corridor-specific mechanisms are proved in the paper; in particular, the list edge coloring ingredient is established from first principles in Appendix D. Beyond standard classical results used throughout graph theory, the only asymptotic external inputs are the two theorems confined to the bulk and sparse regimes. The near-extremal corridor theorem (Proposition 10.5) is elementary and effective.
...
The chordal clique-partition problem of Erdős, Ordman, and Zalcstein asks for the correct extremal order of \(\operatorname{cp}(G)\) on chordal graphs [6]. Their complete-split (indeed threshold) construction shows that the coefficient \(1/6\) is unavoidable. Split graphs form a natural and important subclass of chordal graphs: a graph is split if its vertex set can be partitioned into a clique and an independent set. Earlier work of Chen, Erdős, and Ordman established a sharper asymptotic upper bound for this subclass [3].

This is the third paper in the series on the Erdős–Ordman–Zalcstein clique-partition problem (Erdős Problem #81). Paper I proves a finite fractional split-graph bound, \(|E(G)|-2\nu_3^*(G)\le n^2/6+n\) [13]. Paper II identifies the complete-split terminal family and determines the exact finite maximum of the fractional cover functional over chordal graphs [8]. Papers I and II are fractional and have frozen Lean 4 releases. Paper IV of the series develops a general transfer and rounding interface for integral applications; it is logically independent of the present proof. The present paper is the first integral installment: it strengthens the split-graph estimate to the linear-error scale \(n^2/6+O(n)\).
...
It follows from the dense-graph fractional-to-integral packing theorems of Haxell--Rödl and Yuster [7,9] that every chordal graph satisfies
...
**Table 1. The three proof regimes.** The near-extremal corridor is elementary and effective (Proposition 10.5); the other two regimes each invoke exactly one external asymptotic input.

| Regime | Condition on \(\alpha=q/p\) | Source of slack | External input | Sections |
|---|---|---|---|---|
| Sparse independent side | \(\alpha\to 0\) | dense clique residual admits an exact triangle decomposition | Theorem 2.3 (Dross--BKLO) | §8 |
| Bulk | \(\alpha\) bounded away from \(0\) and \(2\) | quadratic fractional margin (common-profile LP + fractional cloning) | Theorem 2.1 (Haxell--Rödl/Yuster) | §3--4, §9.1 |
| Near-extremal corridor | \(\alpha\to 2\), \(q=2p-s\) | averaged factorization, double-factor polarization, shifted-center gain completion | none (elementary/effective) | §5--7, §9.2--9.3, §10.5 |
...
![A schematic map of the three proof regimes on the parameter axis \(\alpha=q/p\): the sparse regime near \(0\), the bulk away from both endpoints, and the near-extremal corridor near \(2\).](paperIII_figures_v0.9.12/fig2_alpha_regimes_en.png)
...
We use two external asymptotic inputs. Earlier versions cited a third, the list edge coloring theorem; the case required here is proved self-contained in Appendix D. Thus Sections 5--7 and Proposition 10.5 require no additional asymptotic input, though they use standard classical facts such as the edge coloring of complete graphs.

### Theorem 2.1 — Haxell--Rödl/Yuster [7,9]
...
admits a triangle decomposition. This follows from Dross's fractional triangle decomposition theorem [5] (minimum degree \(0.9v\) suffices fractionally) combined with the iterative-absorption theorem of Barber, Kühn, Lo, and Osthus [1], which converts the fractional threshold into an exact decomposition threshold for triangle-divisible graphs, up to \(\varepsilon\) and for sufficiently large order.
...
Dirac's theorem [4] supplies a Hamilton cycle and hence a matching of the required size.
...
In particular, for all sufficiently large members of the sequence, \(\delta(R_0)>p/2\). Dirac's theorem [4] therefore gives a Hamilton cycle, and hence a Hamilton path
...
\]
The same identity holds for \(q=0\), where \(H(p,0,d)=K_p\) and \(\nu_3^*(K_p)=\binom p2/3\). This is an exact closed form for the fractional triangle-packing number of an entire one-parameter family, valid for every \(d\), including the small-\(q\) regime where no integral factorization covers the neighborhood; it may be of independent interest outside the clique-partition problem.
...
Paper V of the series, in preparation, investigates this program. Its intended clique-separator recursion uses the split theorem proved here as the base case and is designed so that the chordal regimes mirror the corresponding ingredients of Sections 3--9: common-profile evaluation, fractional cloning, factorization, and corridor analysis. Until that companion argument is complete, this paragraph records the proposed interface rather than a theorem of the present paper.

## 11.6 Formal-verification perimeter and current status

Papers I and II of the series have frozen Lean 4 / Mathlib releases. The present paper is being prepared under the same release protocol. The author-supplied report records a Mathlib v4.28.0 build comprising 8,057 jobs, zero errors, and zero `sorry`; the public commit and an independent reproduction remain pending.

The formal development separates two layers.

- **Layer X** contains exactly the two external asymptotic inputs, Theorems 2.1 and 2.3, retained as named axioms `AX1` and `AX2` with their hypotheses stated explicitly. In the formalization `AX1` is stated on the cover side (\(\tau_3^*-\nu_3=o(n^2)\)); the proved weak-duality inequality \(\nu_3^*\le\tau_3^*\) is sufficient for the bulk deduction used here. No additional strong-duality assumption is required for that step.
- **Layer E** contains the finite mathematical core recorded by the supplied report as proved from first principles and machine-checked: Theorems 3.1 and 4.2; Lemmas 4.1--7.1; the divisibility correction of Appendix B; the list edge coloring development of Appendix D; the quantitative chains of Sections 9.3 and 10.5; Proposition 10.5; and the final assembly of Theorem 1.1. Two reusable classical results absent from Mathlib — Dirac's Hamiltonicity theorem and a near-perfect matching from minimum degree — are also proved from scratch and are axiom-clean.

The supplied report records the following axiom footprints: Theorem 1.1 and Corollary 1.2 carry `propext`, `Classical.choice`, `Quot.sound`, `AX1`, and `AX2`; the bulk node carries `AX1` in addition to the standard triple; and Proposition 10.5 together with the elementary lemmas and the divisibility correction carry only the standard triple. The same report lists both `AX1` and `AX2` for the sparse-regime node, although the manuscript proof in Section 8 visibly invokes only Theorem 2.3 (`AX2`) among the two named asymptotic inputs. Whether `AX1` is a genuine transitive dependency or an avoidable import remains a release-gate item to be reconciled against the frozen axiom report. The supplied report records no `sorryAx`, `admit`, or `native_decide`, and identifies `AX1` and `AX2` as the only project axiom declarations.

**Table 2. Reported formalization perimeter.** Layer X contains the two external inputs retained as named axioms; Layer E is the finite core recorded as machine-checked relative to those axioms. “Lean report: proved” refers to the author-supplied build and axiom report; independent reproduction and human peer review remain pending.

| Layer | Node | Content | Status |
|---|---|---|---|
| X | `AX1` | Haxell--Rödl/Yuster: \(\tau_3^*-\nu_3=o(n^2)\) | Named axiom (external) |
| X | `AX2` | Dross--BKLO: triangle-divisible, \(\delta\ge(0.9+\varepsilon)n\) \(\Rightarrow\) triangle decomposition | Named axiom (external) |
| E | Thm 3.1 | common-profile exact fractional value \(F(p,q,d)\) | Lean report: proved |
| E | Thm 4.2 | unified fractional margin | Lean report: proved |
| E | Lemmas 4.1--7.1 | cloning, factorization, polarization, shifted-center | Lean report: proved |
| E | App. B | divisibility correction (Dirac ordering + parity) | Lean report: proved |
| E | App. D | list edge coloring (Galvin/König, self-contained) | Lean report: proved |
| E | Prop 10.5 | explicit corridor bounds (effective, unconditional) | Lean report: proved |
| E | Bulk node (§9.1) | fractional-to-integral in the bulk | Lean report: proved, modulo `AX1` |
| E | Sparse node (§8) | dense residual decomposition | Lean report: proved, reported modulo `AX1`, `AX2`; manuscript proof visibly uses `AX2`; dependency reconciliation pending |
| E | **Thm 1.1** | main theorem (assembly) | **Lean report: proved, modulo `AX1`, `AX2`** |

No computation or regression script is a premise of the proof. The exact-arithmetic and integer-programming checks are supplementary audits. The supplied audit summary reports, under one counting convention, 46,390 exact-arithmetic and LP checks and 91 exact ILP checks, for a total of 46,481 checks, with no reported discrepancy.

The public-preprint status line will be tied to the frozen release commit, the axiom report, and the Lean toolchain and manifest. Independent reproduction of the build from that public commit, human peer review, and the prior-art assessment remain pending; on those grounds the wording "formally verified" is reserved for the frozen release.
...
### Corollary 12.3 — Effective corridor algorithm

In the near-extremal corridor covered by Proposition 10.5 (explicit \(p_0,s_0\)), the proof is
constructive: the one- and double-factorizations of \(K_p\), the reserved-vertex selection, the
list edge coloring (Appendix D, whose kernel/stable-pairing proof is itself an algorithm), and
the matching extraction all run in polynomial time. Hence there is a deterministic
polynomial-time algorithm that, for split graphs in the effective corridor, outputs a clique
partition of size \(n^2/6+2n\) with the explicit constant of Proposition 10.5. Outside the
corridor the guarantee currently relies on the non-constructive bulk input (Theorem 2.1); a
fully effective global algorithm is left to future work.
...
Two pieces of evidence support a Simonovits-type stability statement. First, an exhaustive
enumeration at \(n=9\) (all split graphs with \(|K|=3\)) shows that graphs within an additive
constant of the maximum are within a bounded edit distance of the complete-split family, while
genuinely dispersed profiles fall a linear amount below the maximum. Second, the fractional
...
The common-profile and cloning ideas suggest one possible approach, but the relevant local orbit programs and integral rounding mechanisms become substantially more complicated. Design and decomposition methods such as those developed by Keevash [15] and the regular-slice framework of Allen, Böttcher, Cooley, and Mycroft [16] may be useful tools; they are not ingredients of the present proof.
...
The intended working release package for this draft is organized as follows.

```text
01_MANUSCRIPT/
    PAPER_III_split_lineal_v0.9.12_editorial_review_en.md
02_PROOF_CONTROL/
    LEDGER.md
    LEDGER_INCREMENTAL_formalization_draft.md
    AUDIT_PAPER_C.md
03_FORMALIZATION/
    lean/                         (Lean 4 / Mathlib v4.28.0 sources)
    lakefile.toml
    lean-toolchain
    lake-manifest.json
    BUILD_STATUS.md               (temporary until the release build is frozen)
...
The manuscript is the mathematical source of truth. `LEDGER.md` is the authoritative dependency specification for the Lean development; the incremental ledger records only the transition from v0.9.1 to this draft. The final release package will add the frozen commit, build log, axiom report, and SHA-256 manifest. Before release, the script index in this section must be reconciled with the more detailed audit inventory in Appendix C; no script name or check count should be treated as frozen until that reconciliation is complete.
...
The scripts listed in the working release package are supplementary regression tests for the explicit corridor constants. Their outputs are not used as hypotheses, reductions, or proof steps anywhere in the manuscript. The script inventory remains subject to reconciliation with Appendix C before the release package is frozen. Historical exploratory computations are not part of the logical proof package.

The public release will be made through the series repository at a frozen tag and commit. Manuscripts, Markdown, LaTeX, PDFs, and related written materials are intended to be distributed under CC BY-NC 4.0. Author-owned verification and regression code is intended to be distributed under the PolyForm Noncommercial License 1.0.0; commercial software licensing is available from the author. Third-party dependencies retain their own licences.

**Open release gates.** Before public release, the package index and Appendix C must be reconciled; the Lean toolchain, manifest, build log, and axiom report must be frozen at the public commit; the reported computational totals must be regenerated from the packaged scripts; and an independent build reproduction, human peer review, and prior-art review remain pending.
---
...
[11] D. Gale and L. S. Shapley, “College admissions and the stability of marriage,” *The American Mathematical Monthly* **69** (1962), no. 1, 9–15.

[12] D. König, “Über Graphen und ihre Anwendung auf Determinantentheorie und Mengenlehre,” *Mathematische Annalen* **77** (1916), 453–465.
...
[16] P. Allen, J. Böttcher, O. Cooley, and R. Mycroft, “Tight cycles and regular slices in dense hypergraphs,” *Journal of Combinatorial Theory, Series A* **149** (2017), 30–100.
