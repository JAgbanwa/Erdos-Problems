...
**Author manuscript:** v1.1.6 near-final editorial candidate for external adversarial AI audit. This candidate supersedes v1.1.5. All v1.1.6 changes are editorial and concern verification status, dependency presentation, release-layer organization, and terminology; theorem statements, definitions, constants, displayed identities, and proofs are unchanged relative to v1.1.5. Version v1.1 sharpens the short-corridor constant in Proposition 10.5 from \(C_{\mathrm{corr}}=2\) to \(3/2\), with short-corridor threshold \(p_0=37\); the combined corridor summary retains its separate threshold \(p_0=2304\), and adds packing-form corollaries of Sections 5 and 7. The three-regime architecture, the remaining material in Sections 2–9, and the appendices are retained. The Lean development adds `short_corridor_sharp`, `s_le_p_sub_one`, `factorization_assignment_packing`, `double_factorization_packing`, and `reserved_gain_packing_bound_subset`; the core of Theorem 1.1 relative to `AX1` and `AX2` is retained.  
**Date:** July 28, 2026  
**Status:** near-final editorial candidate for external adversarial AI audit. The Paper III Lean v1.0 freeze builds successfully against Lean 4 / Mathlib v4.28.0. It verifies the internal finite reduction, construction, and arithmetic layers relative to exactly two named external asymptotic inputs: `AX1` (Haxell--Rödl/Yuster) and `AX2` (Dross + Barber--Kühn--Lo--Osthus). No other project axiom is present. The project also reports completion of the associated computational regression tests without discrepancy. Section 11.6 records the dependency perimeter and Section 13 records the freeze archive. Independent reproduction of the frozen archive, external peer review, and the prior-art assessment remain separate release gates.
...
All corridor-specific mechanisms are proved in the paper; in particular, the list edge coloring ingredient is established from first principles in Appendix D. The formal proof is conditional only on two standard asymptotic inputs: `AX1`, the Haxell--Rödl/Yuster fractional–integral packing theorem used in the bulk regime, and `AX2`, the Dross plus Barber--Kühn--Lo--Osthus decomposition input used in the sparse regime. The Lean formalization verifies the internal reduction and arithmetic layers relative to these inputs. The near-extremal corridor theorem (Proposition 10.5) is elementary and effective and depends on neither `AX1` nor `AX2`.
...
**Table 1. The three proof regimes and their formal dependencies.** The Lean freeze verifies the internal argument in each regime; the bulk and sparse regimes each retain one named external input, while the near-extremal corridor is axiom-clean beyond Lean's standard foundations.

| Regime | Condition on \(\alpha=q/p\) | Source of slack | Lean status | External dependence | Sections |
|---|---|---|---|---|---|
| Sparse independent side | \(\alpha\to 0\) | dense clique residual admits an exact triangle decomposition | internal argument verified relative to `AX2` | `AX2` (Dross + Barber--Kühn--Lo--Osthus) | §8 |
| Bulk | \(\alpha\) bounded away from \(0\) and \(2\) | quadratic fractional margin (common-profile LP + fractional cloning) | internal argument verified relative to `AX1` | `AX1` (Haxell--Rödl/Yuster) | §3--4, §9.1 |
| Near-extremal corridor | \(\alpha\to 2\), \(q=2p-s\) | averaged factorization, double-factor polarization, shifted-center gain completion | verified with standard Lean foundations | none | §5--7, §9.2--9.3, §10.5 |
...
### Theorem 2.1 — Haxell--Rödl/Yuster (`AX1`) [11,17]
...
### Theorem 2.3 — Dense triangle decomposition (`AX2`)
...
The following corollaries restate results from Sections 5 and 7 directly in packing language. They are formalized as `factorization_assignment_packing`, `double_factorization_packing`, and `reserved_gain_packing_bound_subset`. All three are unconditional: none uses Theorem 2.1/`AX1` or Theorem 2.3/`AX2`.
...
\[
\nu_3(G)\ \ge\ \binom b2+\binom\rho2-\frac{(2b-1)A_J-A_{2,J}}{2q_J}+(1-\theta_R)\frac{(q_J-r_b)B_J}{q_J},
\]
with \(A_J=\sum_{i\in J}t_i\), \(A_{2,J}=\sum_{i\in J}t_i^2\), \(B_J=\sum_{i\in J}g_i\). The construction of Section 7 is applied to the retained hosts \(J\) only, so it survives deleting deficient hosts; the cover cost of the excluded hosts is not part of this packing bound and belongs to any covering application built on it.
...
The frozen snapshot at `PAPER_III/05_formalization/lean_v1.0_freeze` builds successfully against Lean 4 / Mathlib v4.28.0. The recorded build completed 8,060 jobs with zero errors. Its trusted project layer contains exactly the two named external inputs `AX1` and `AX2`; no other project axiom is present. The freeze log records 64 linter warnings, which concern style or unused declarations and do not affect kernel elaboration.
...
**Named external inputs.** The freeze documentation identifies `AX1` with the Haxell--Rödl/Yuster fractional–integral triangle-packing gap and `AX2` with the Dross plus Barber--Kühn--Lo--Osthus asymptotic triangle-decomposition input. Lean verifies the internal deductions relative to these statements; it does not replace or reprove the two external theorems. The dependency split is bulk–`AX1`, sparse–`AX2`, and corridor–none.

The frozen dependency report records `AX1` and `AX2` for Theorem 1.1 and Corollary 1.2, `AX1` for the bulk node, and `AX2` for the sparse node. Proposition 10.5, the corridor-specific lemmas, the divisibility correction, and the v1.1 packing-form corollaries carry only `propext`, `Classical.choice`, and `Quot.sound`. The freeze records no `sorryAx`, `admit`, `native_decide`, or additional project axiom declaration.

**Table 2. Frozen formalization perimeter.** Layer X contains the two named external inputs; Layer E is the finite core verified by the Lean v1.0 freeze relative to those inputs. Independent archive reproduction and human peer review remain pending.

| Layer | Node | Content | Status |
|---|---|---|---|
| X | `AX1` | Haxell--Rödl/Yuster: \(\tau_3^*-\nu_3=o(n^2)\) | Named external input |
| X | `AX2` | Dross + Barber--Kühn--Lo--Osthus: triangle-divisible, \(\delta\ge(0.9+\varepsilon)n\) \(\Rightarrow\) triangle decomposition | Named external input |
...
| E | Prop 10.5 (sharp) | `short_corridor_sharp`, `s_le_p_sub_one`: \(C_{\mathrm{corr}}=3/2\), \(p_0=37\) | Lean report: proved (standard triple) |
| E | §5/§7 packing-native | `factorization_assignment_packing`, `double_factorization_packing`, `reserved_gain_packing_bound_subset` | Lean report: proved (standard triple) |
| E | Bulk node (§9.1) | fractional-to-integral in the bulk | Lean freeze: proved, relative to `AX1` |
| E | Sparse node (§8) | dense residual decomposition | Lean freeze: the exposed node `E_8` depends on `AX1` and `AX2`; the very-sparse core lemma is `AX2`-only |
| E | **Thm 1.1** | main theorem (assembly) | **Lean freeze: proved, relative to `AX1`, `AX2`** |

The regime labels above summarize the mathematical architecture rather than the transitive dependency of every exposed Lean node. In particular, the sparse argument is organized around the `AX2` decomposition input, but `E_8` dispatches an intermediate alpha-range through the bulk lemma `E_4_3` and therefore inherits `AX1` as well.

No computation or regression script is a premise of the proof. The exact-arithmetic and integer-programming checks are supplementary audits. The supplied audit summary reports 46,390 exact-arithmetic and LP checks and 91 exact ILP checks, for a total of 46,481 checks, under its current counting convention and with no reported discrepancy. The external adversarial audit should independently verify this counting convention from the supplied scripts and ledgers.

The public-preprint status line will be tied to the frozen release commit, the axiom report, and the Lean toolchain and manifest. Independent reproduction of the build from that public commit, human peer review, and the prior-art assessment remain pending; until those gates close, Theorem 1.1 is described only as author-reported machine-checked relative to the named axioms `AX1` and `AX2`.
...
The mathematical source is this manuscript. The formal source snapshot is frozen separately at `PAPER_III/05_formalization/lean_v1.0_freeze`; its full build log is stored under `gate_logs/lake_build_full.log`. The source archive is `PAPER_III_LEAN_v1.0_FREEZE_sources.zip`, with SHA-256 `060957e6b8d54779844dc6adf7cc7c3b8446fc17a87aa8d7a437e9d9d1001b78`. The freeze is separate from the earlier working folder under `05_formalization/lean`, which was left unchanged.

`LEDGER.md` remains the dependency specification for the formal development, while Appendix C describes the supplementary computational audits. Neither the regression scripts nor their outputs are proof premises. The public release commit and independent reproduction of the frozen archive remain separate from this editorial candidate.
...
The public release will be made through the series repository at a frozen tag and commit. Manuscripts, Markdown, LaTeX, PDFs, and related written materials are intended to be distributed under CC BY-NC 4.0. Author-owned verification and regression code is intended to be distributed under the PolyForm Noncommercial License 1.0.0; commercial software licensing is available from the author. Third-party dependencies retain their own licenses.

**Open release gates.** The next gate is the planned external adversarial AI audit against this near-final manuscript and the frozen Lean archive. Independent reproduction of the archive, human peer review, prior-art review, and publication of a final public tag and commit remain pending.
---
...
[15] J. P. Traverso Gianini, “Affine profile reduction for fractional triangle packings in split graphs” (Paper I in the series), preprint v1.1, July 2026.

[16] J. P. Traverso Gianini, “Complete-split extremizers for a fractional triangle-cover functional on chordal graphs” (Paper II in the series), preprint v1.1, July 2026.
