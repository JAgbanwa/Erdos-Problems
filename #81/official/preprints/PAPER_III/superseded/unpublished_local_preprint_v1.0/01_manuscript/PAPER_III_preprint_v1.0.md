...
This is the third paper in the series on the Erdős–Ordman–Zalcstein clique-partition problem (Erdős Problem #81 [3]). Paper I proves a finite fractional split-graph bound, \(|E(G)|-2\nu_3^*(G)\le n^2/6+n\) [15]. Paper II identifies the complete-split terminal family and determines the exact finite maximum of the fractional cover functional over chordal graphs [16]. Papers I and II are fractional and have frozen Lean 4 releases. Paper IV of the series develops a general transfer-and-rounding interface for integral applications; it is logically independent of the present proof. The present paper is the first integral installment: it strengthens the split-graph estimate to the linear-error scale \(n^2/6+O(n)\).
...
```text
01_MANUSCRIPT/
    PAPER_III_split_lineal_v0.9.16_editorial_review_en.md
02_PROOF_CONTROL/
    LEDGER.md
