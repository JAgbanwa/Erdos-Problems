...
**Preprint draft:** version 1.2. This manuscript and its accompanying formal artifact form one self-contained review package and the candidate for the first formal public release of Paper III.

**Draft package date:** August 21, 2026  
**Status:** `EDITORIAL_DRAFT_WITH_OPEN_GATES`; unpublished. The accompanying Lean 4 / Mathlib development and the remaining release gates are summarized in Sections 11.6 and 13. A consolidated immutable local freeze and frozen axiom output are now included in the draft package and have passed the internal author-side audit. Independent reproduction, external adversarial audit, external peer review, and final prior-art/novelty review remain required before release.

**Internal prior-art and novelty assessment.** The internal literature audit found no earlier proof improving the Chen--Erdős--Ordman split-graph coefficient (3/16) to the sharp coefficient (1/6) with linear error. On that internal record, the paper resolves the split-graph case of Erdős Problem #81 at (n^2/6+O(n)); the full chordal problem remains open. This is an internal editorial assessment, not a substitute for independent prior-art review, which remains a release gate.
...
The formalization discussion is confined to this section and the release record in Section 13. The current local source separates the unconditional final assembly from its downstream interfaces. `PaperIII.Theorem_1_1_Final` discharges the two interfaces used by the paper and exports Theorem 1.1 and Corollary 1.2. `PaperIII.PaperImprovements` contains the paper-facing certificates; `PaperIII.PublicAPI`, `PaperIII.OfPartition`, and `PaperIII.Obstructions` expose downstream interfaces. The project reports the following axiom footprint for these theorem surfaces:

```text
[propext, Classical.choice, Quot.sound]
```

The final assembly uses explicit bridges to the Haxell--Rödl/Yuster and dense-decomposition formalizations. The cited asymptotic theorems remain in the manuscript for mathematical provenance. The accompanying package contains the consolidated local build and declaration-level axiom report specified in Section 13. This closes the internal freeze gate; independent archive reproduction remains an external release gate.
...
**Table 2. Formalization surfaces.** Statuses in this table are frozen local-project records; they are not a substitute for independent reproduction.

| Surface | Purpose | Current local status |
|---|---|---|
| `PaperIII` | legacy aggregate/compatibility root | included in the consolidated multi-target build; final/public surfaces are built and imported explicitly |
| `PaperIII.MainNibble` | theorem and corollary assembly through explicit AX1/AX2 interfaces | included in the successful consolidated build |
| `PaperIII.Theorem_1_1_Final` | unconditional discharge of AX1/AX2 and final theorem surfaces | included; foundational-only footprint recorded |
| `PaperIII.PaperImprovements` | paper-facing corollaries and selected auxiliary certificates | included in the successful consolidated build |
| `PaperIII.PaperImprovementsGate` | axiom gate for paper-facing byproducts | refreshed and included in the consolidated gate run |
| `PaperIII.PublicAPI` | downstream API and Paper V entry points | included; foundational-only footprint recorded |
| `PaperIII.OfPartition` | `SplitGraph.ofPartition`, graph-isomorphism invariance, and the split-partition entry point | included; foundational-only footprint recorded |
| `PaperIII.Obstructions` | density-obstruction certificates for Remark 2.3\(^{\prime}\) | included; foundational-only footprint recorded |
| `FREEZE_REPORT.md` and `ESCAPE_HATCH_ASSESSMENT.md` | frozen build, axiom and raw-token assessment | present in the package and reviewed internally |
...
The consolidated immutable freeze records a successful build of 8,717 jobs, and 14 declaration-level axiom checks report only `propext`, `Classical.choice`, and `Quot.sound`. The internal audit reviewed those frozen records without rerunning Lean.
...
The mathematical source is this manuscript. The manuscript-facing formal surfaces are `PaperIII.Theorem_1_1_Final`, `PaperIII.PaperImprovements`, `PaperIII.PublicAPI`, `PaperIII.OfPartition`, and `PaperIII.Obstructions`. `PaperIII.PaperImprovementsGate` is the byproduct gate. The local v1.2 release-preparation package freezes these surfaces alongside the aggregate root and builds them as an explicit multi-target set. It records its status in `FREEZE_REPORT.md`, `ESCAPE_HATCH_ASSESSMENT.md`, and `gate_logs/`.

The delivered formal archive is `PAPER_III_lean_v1.2_freeze.zip`, with SHA-256 `9bff12f0c8279ae4485de960c01be322a423d8fd2f17592ebb7327b2c890fcb8`. It is stored under `PAPER_III/05_formalization/lean_v1.2_freeze` together with its sidecar, manifests, and recorded logs.

The available local verification record is summarized below.

**Table 3. Frozen local verification record and remaining external gap.**

| Check | Log or command | Reported result |
|---|---|---|
| consolidated source freeze | `gate_logs/BUILD_LOG.txt` and `BUILD_EXIT.txt` | exit 0; `Build completed successfully (8717 jobs)` |
| theorem-level axiom gates | `gate_logs/AXIOMS_REPORT.txt` and `AXIOMS_EXIT.txt` | exit 0; 14 surfaces; only `propext`, `Classical.choice`, `Quot.sound` |
| headline final theorem | `#print axioms PaperIII.Theorem_1_1` | foundational-only footprint recorded |
| AX1 and AX2 discharges | `#print axioms PaperIII.AX1_holds`; `#print axioms PaperIII.AX2_holds` | foundational-only footprints recorded |
| escape-hatch assessment | `gate_logs/ESCAPE_HATCH_SCAN.txt`; `ESCAPE_HATCH_ASSESSMENT.md` | no active `sorry` warning; three raw matches are documented block comments |

The frozen package records a single build of the aggregate root, final assembly, public interfaces, obstruction module, and refreshed paper-improvement gate. Its external reproduction should rerun at least

```text
lake build PaperIII PaperIII.Theorem_1_1_Final PaperIII.PublicAPI PaperIII.OfPartition PaperIII.Obstructions PaperIII.PaperImprovementsGate
#print axioms PaperIII.Theorem_1_1
#print axioms PaperIII.AX1_holds
#print axioms PaperIII.AX2_holds
```

and compare the verbatim output with the frozen logs, component commit identifiers, and SHA-256 inventories. The internal audit treats `[propext, Classical.choice, Quot.sound]` as a verified frozen local fact, not as an independently reproduced result.
...
**Open release gates:** independent reproduction of the frozen package, including a fresh build and theorem-level axiom checks; external adversarial audit; human peer review; a prior-art review covering recent and in-preparation work; and publication of the final public tag and commit.
