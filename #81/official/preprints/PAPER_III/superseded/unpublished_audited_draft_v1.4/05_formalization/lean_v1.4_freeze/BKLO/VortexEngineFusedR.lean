/-
# BKLO §10 — the vortex recursion, from the **repaired** fused interface.

`BKLO/VortexEngineFused.lean` runs the vortex from `BKLO.VortexReservoirEngine`; but that
interface is false (`BKLO.not_vortexReservoirEngine`), so that route is vacuous.  This file redoes
the two steps of the derivation from the repaired interface of `BKLO/ReservoirRepaired.lean`:

* `coverDownStepR_of_reservoirClauseR` — one cover-down step, from the repaired reservoir clause
  and the nibble in its maximum-degree form.  Compared with
  `BKLO.coverDownStep_of_reservoirClause` it takes one extra hypothesis (the between-levels
  density, which the vortex supplies) and delivers one extra conclusion (the damage bound at the
  scale of the protected level `W''`), and its engine room is `BKLO.coverDown_of_reservoirR`;
* `coverDown_vortex_denseR` — the recursion down the vortex.  Its state carries two more
  invariants than the one of `BKLO.coverDown_vortex_dense`:

  - *(between levels)* every vertex of the current level `W` has at least `c₁|W'|` neighbours in
    the next level `W'` — this is the hypothesis the repaired reservoir clause needs, and it is
    re-established after each step from the density of the level after next minus the `W''`-scale
    damage bound;
  - *(bottom)* every vertex of the next level `W'` has at least `f(|U|)|U|` neighbours in the
    bottom set `U` — this is what feeds the previous invariant on the last step, when the level
    after next *is* `U`; it survives each step because the step touches no edge inside `W''ered`
    and `U ⊆ W''`.

Everything here is `sorry`-free.
-/
import BKLO.CoverDownFusedR
...
/-- **One cover-down step, from the repaired reservoir clause and the nibble.**  As
`BKLO.coverDownStep_of_reservoirClause`, with the between-levels density hypothesis added and the
`W''`-scale damage bound added to the conclusion. -/
theorem coverDownStepR_of_reservoirClauseR
    ...
    (hres : ReservoirClauseR ε η f n₂ K) :
  ...
  classical
  intro V _ W W' W'' F hcard hW'W hW''W' hr1 hr2 hr3 hFW hdiv hdeg hclean hbetween
  obtain ⟨R, hRF, hRcross, hRdeg, hapex, hlink⟩ :=
    hres W W' W'' F hcard hW'W hW''W' hr1 hr2 hr3 hFW hdiv hdeg hclean hbetween
...
/-- Edges from a vertex of the protected level into the bottom set survive a cover-down step. -/
theorem resLink_subset_of_keep {F F₁ : Finset (Sym2 V)} {U W'' : Finset V} (hUW'' : U ⊆ W'')
    (hkeep : F ∩ cliqueEdges W'' ⊆ F₁) {v : V} (hv : v ∈ W'')
    (hnd : ∀ e ∈ F, ¬ e.IsDiag) : resLink F U v ⊆ resLink F₁ U v := by
  classical
  intro a ha
  obtain ⟨haU, haF⟩ := mem_resLink.1 ha
  refine mem_resLink.2 ⟨haU, hkeep (Finset.mem_inter.2 ⟨haF, mem_cliqueEdgesV.2 ⟨?_, hnd _ haF⟩⟩)⟩
  intro z hz
  rcases Sym2.mem_iff.1 hz with rfl | rfl
  exacts [hv, hUW'' haU]

/-- **Running the cover-down along the whole vortex, with the repaired invariants.**  As
`BKLO.coverDown_vortex_dense`, with the between-levels density and the density into the bottom set
carried along the recursion. -/
theorem coverDown_vortex_denseR {c₁ γ : ℝ} {f : ℕ → ℝ} {n₂ n₀ K : ℕ} {U : Finset V} (hK : 2 ≤ K)
    ...
    (hdesc : ∀ (W : Finset V) (E : Finset (Sym2 V)) (m : ℕ),
      U ⊆ W → U.card ≤ m → 2 * m ≤ W.card → W.card ≤ K * K * m → E ⊆ cliqueEdges W →
      ...
      obtain ⟨W'', hUW'', hW''W', hW''r1, hW''r2, hW''disj, hres''⟩ :
        ...
        · push_neg at hsmall
          ...
          obtain ⟨W'', hUW'', hW''W', hcard'', hres''⟩ :=
            hdesc W' (F ∩ cliqueEdges W') (W'.card / K) hUW' ham h2m hmm
              Finset.inter_subset_right hclean
