/-
# The one-link class-matched demand at the **re-sized** reservoir

`BKLO.TwoSidedUsedClassMatchedQuarterPairing` (`BKLO/TwoSidedUsedClassMatchedQuarter.lean`) is the
residual of AX2 §10 at the old sizing of the reservoir: classes balanced to a *quarter*, so the
equalized trace is `c = q - q/4`, and a perturbation scale of `cleanEta ε K`, so the adversary may
move a quarter of a class.  `BKLO/AX2CellStepRepair.lean` shows that at that sizing the quarter
condition of `BKLO.exists_cell_balanced_leftovers` has no room at all
(`BKLO.cell_quarter_condition_fails_of_quarter_equalized`), and prescribes the re-sizing.

This file states the same demand at the re-sized reservoir, which
`BKLO/ReservoirDesignTwoSidedEighth.lean` and `BKLO/GridPairingTwoSidedEighth.lean` construct:

* the design is `BKLO.IsGridTwoSidedReservoirEighth`, so the trace satisfies `7 q ≤ 8 c`;
* the perturbation is bounded by a **thirty-second** of a class on each side,
  `32 * (resLink R W' u \ X u).card ≤ t`, which `BKLO.twoSided_perturbation_eighth` supplies from
  the scale `BKLO.cleanEtaEighth`.

Together these are exactly the hypotheses of `BKLO.exists_cell_balanced_leftovers_of_resized`, so
the cell prescription of AX2 §10 is available at this demand and not at the old one.

* `BKLO.TwoSidedUsedClassMatchedResizedPairing` — the demand;
* `BKLO.gridPairingClauseTwoSidedEighth_of_usedClassMatchedResized` — the pairing clause from it;
* `BKLO.triangle_decomposition_of_inputs_and_twoSidedUsedClassMatchedResized` — the main theorem of
  the AX2 half from it.
...
/-- **The one-link class-matched pairing demand at the re-sized reservoir.**  This is
`BKLO.TwoSidedUsedClassMatchedQuarterPairing` with the two re-sizings of
`BKLO/AX2CellStepRepair.lean`: the design is balanced to an eighth (so `7 q ≤ 8 c`) and the
perturbation of a link is at most a thirty-second of a class on each side. -/
def TwoSidedUsedClassMatchedResizedPairing : Prop :=
...
/-- **The pairing clause at the re-sized two-sided design, from the re-sized one-link
class-matched pairing demand.** -/
theorem gridPairingClauseTwoSidedEighth_of_usedClassMatchedResized
    (hpair : TwoSidedUsedClassMatchedResizedPairing)
  ...
  obtain ⟨ρ, σ, Inv, hρlt, hσlt, hfib, hInv0, hInvSpread, hstep⟩ :=
    hpair X hgrid8 hnd hW'W hq hc hqc8 hε hε' hK hbig512
...
/-- **The remaining residual of AX2 §10 at the re-sized design, from the re-sized one-link
class-matched pairing demand.** -/
theorem gridPairingResidualTwoSidedEighth_of_usedClassMatchedResized
    (hpair : TwoSidedUsedClassMatchedResizedPairing) : GridPairingResidualTwoSidedEighth := by
  intro ε hε hε' K hK hKε
  refine ⟨max ⌈(16 : ℝ) / ε⌉₊ (5120 * (gridSize ε K * gridSize ε K) * (K * K)),
    fun f n₂ hn₂ _hwin => gridPairingClauseTwoSidedEighth_of_usedClassMatchedResized hpair hε hε'
      (by omega) ?_ ?_⟩
...
/-- **The §10 interface, from the re-sized one-link class-matched pairing demand.** -/
theorem vortexReservoirEngineR4_of_twoSidedUsedClassMatchedResized
    (hpair : TwoSidedUsedClassMatchedResizedPairing) : VortexReservoirEngineR4 :=
  vortexReservoirEngineR4_of_gridPairingResidualTwoSidedEighth
    (gridPairingResidualTwoSidedEighth_of_usedClassMatchedResized hpair)

/-- **Main theorem (AX2 half of Erdős #81), from the three classical inputs and the re-sized
one-link class-matched pairing demand.** -/
theorem triangle_decomposition_of_inputs_and_twoSidedUsedClassMatchedResized
