/-
# The one-link class-matched demand, in the regime in which it is used, **with the perturbation
bounded against the class size**

`BKLO.TwoSidedUsedClassMatchedInvariantPairing` (`BKLO/TwoSidedUsedClassMatched.lean`) is **false**
— `BKLO.not_twoSidedUsedClassMatchedInvariantPairing`
(`BKLO/TwoSidedUsedClassMatchedObstruction.lean`).  What it lets the adversary do and no sweep ever
does is *perturb a link by whole classes*: its only constraint on the perturbation `n` is the
degree margin `12 n + 8 m ≤ (2h-1) c`, which allows `n` to be about `h c / 12`, i.e. about `h/12`
whole classes.  Deleting `D` classes of the column part of a link forces `(D-1) c` places of its
row part to be leftovers paired inside the row part, and a sweep of one grid row then overruns
`BKLO.ExcLedgerSpread`.

At the only place the demand is applied,
`BKLO.gridPairingClauseTwoSided_of_usedClassMatchedInvariant`, the link system satisfies
`BKLO.GridPairingClauseTwoSided`'s own hypotheses `hXadd`, `hXdel`, which by
`BKLO.twoSided_perturbation_quarter` give

```
4 * (X u \ resLink R W' u).card ≤ t,   4 * (resLink R W' u \ X u).card ≤ t
```

— the perturbation of a link is less than a **quarter of one class**, so not a single class can be
deleted.  This file states the demand with exactly those two hypotheses added, and rebuilds the
whole AX2 chain from it; nothing else changes, and the counterexample of
`BKLO/TwoSidedUsedClassMatchedObstruction.lean` no longer applies.

Everything here is `sorry`-free.  The demand `BKLO.TwoSidedUsedClassMatchedQuarterPairing` itself
is *not* proved here: it is the remaining residual of AX2 §10 in this development.
-/
...
sweep's own forbidden edges, in the regime in which it is used, with the perturbation of the link
bounded against the class size.**

This is `BKLO.TwoSidedUsedClassMatchedInvariantPairing` with the defect that refutes it removed:
the perturbation of the link is at most a quarter of a class, which is what
`BKLO.twoSided_perturbation_quarter` provides at the only place the demand is applied.  Without
that bound the demand is false — `BKLO.not_twoSidedUsedClassMatchedInvariantPairing`. -/
def TwoSidedUsedClassMatchedQuarterPairing : Prop :=
...
invariant against the sweep's own forbidden edges and with the perturbation bounded against the
class size.** -/
theorem gridPairingClauseTwoSided_of_usedClassMatchedQuarter
    (hpair : TwoSidedUsedClassMatchedQuarterPairing)
  ...
  have hJstep : IsSpreadStepUsed ε K W W' W'' F R X c J := by
    ...
    obtain ⟨p, e, h1, h2, h3, h4, h5, h6⟩ :=
      hstep S g₀ Exc u n m U hu hXu hXeven'
        (twoSided_perturbation_quarter hgrid hKpos (hXadd u hu))
        (twoSided_perturbation_quarter hgrid hKpos (hXdel u hu))
...
pairing with an invariant, against the sweep's own forbidden edges, with the perturbation bounded
against the class size.** -/
theorem gridPairingResidualTwoSided_of_usedClassMatchedQuarter
    (hpair : TwoSidedUsedClassMatchedQuarterPairing) : GridPairingResidualTwoSided := by
  intro ε hε hε' K hK hKε
  refine ⟨max ⌈(16 : ℝ) / ε⌉₊ (5120 * (gridSize ε K * gridSize ε K) * (K * K)),
    fun f n₂ hn₂ _hwin => gridPairingClauseTwoSided_of_usedClassMatchedQuarter hpair hε hε'
      (by omega) ?_ ?_⟩
...
sweep's own forbidden edges and with the perturbation bounded against the class size.** -/
theorem vortexReservoirEngineR4_of_twoSidedUsedClassMatchedQuarter
    (hpair : TwoSidedUsedClassMatchedQuarterPairing) : VortexReservoirEngineR4 :=
  vortexReservoirEngineR4_of_gridPairingResidualTwoSided
    (gridPairingResidualTwoSided_of_usedClassMatchedQuarter hpair)
...
edges, with the perturbation bounded against the class size.**  This is the form of the demand
that survives every counterexample known here: the forbidden set is the sweep's own
(`BKLO.UsedForbidden`), the regime hypotheses of the application site are available, and the
perturbation of a link is at most a quarter of a class — which is what
`BKLO.not_twoSidedUsedClassMatchedInvariantPairing` shows the previous form was missing. -/
theorem triangle_decomposition_of_inputs_and_twoSidedUsedClassMatchedQuarter
