...
which for `k` large is an exponentially small fraction of all `t`-subsets.

A dependency-free hypergeometric concentration toolkit (Markov on factorial moments over
`Finset.powersetCard`), useful in probabilistic combinatorics over uniformly random subsets.

* `Contrib.Hypergeometric.card_deviant_le_pow` (the tail bound), with the moment identity
  `sum_choose_card_inter` and `card_powersetCard_filter_superset`.
...
namespace Contrib.Hypergeometric
