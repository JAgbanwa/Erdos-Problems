/-
# §10 and the main theorem, from the **repaired** fused interface `BKLO.VortexReservoirEngineR4`.

`BKLO/MainRepaired3.lean` derives the main theorem from `BKLO.VortexReservoirEngineR3`, whose
reservoir clause `BKLO.ReservoirClauseR` is false (`BKLO.not_reservoirClauseResidual`): the clause
is asked of protected levels `W''` too small for its own damage bound `γ|W''| < 1` to allow the one
edge into `W''` that a covered crossing edge into `W''` costs.

This file redoes the derivation from `BKLO.VortexReservoirEngineR4` of `BKLO/ReservoirLarge.lean`,
whose reservoir clause carries the hypothesis that `W''` is empty or of size at least `n₂`.  The
proof is the proof of `BKLO.nearOptimalConclusion_of_fusedR3`, with that hypothesis threaded
through the cover-down step and discharged at the two places where the vortex recursion applies it
(`BKLO.coverDown_vortex_denseR2Large`).
...
interface `BKLO.VortexReservoirEngineR4`.
-/
...
/-- **§10, from the repaired fused interface and the classical inputs.** -/
theorem nearOptimalConclusion_of_fusedR4 (hDross : FracTriangleThreshold)
    (hNib : FracToApproxMaxDeg) (hEng : VortexReservoirEngineR4) : NearOptimalConclusion := by
  classical
  intro ε hε
  ...
  have hCD : ∀ {V : Type} [DecidableEq V] (W W' W'' : Finset V) (F : Finset (Sym2 V)),
    ...
    coverDownStepR_of_reservoirClauseR4 (by linarith) hKε hNn₂ hnib hres
...
theorem nearOptimalDecomp_of_fusedR4 (hNib : FracToApproxMaxDeg) (hEng : VortexReservoirEngineR4) :
    NearOptimalDecomp :=
  fun hDross _ _ => nearOptimalConclusion_of_fusedR4 hDross hNib hEng

/-- **Main theorem (AX2 half of Erdős #81), from the classical inputs and the repaired §10
interface.** -/
theorem triangle_decomposition_of_inputs_repaired4
    (hDross : FracTriangleThreshold) (hNib : FracToApproxMaxDeg) (hDirac : PerfectMatchingDirac)
    (hEng : VortexReservoirEngineR4) :
    ∀ ε : ℝ, 0 < ε → ∃ n₀ : ℕ,
  ...
  triangle_decomposition_of_inputs hDross (fracToApprox_of_maxDeg hNib) hDirac
    (nearOptimalDecomp_of_fusedR4 hNib hEng)
