...
This is the counting that turns the *codegree* hypothesis (iii) of BKLO Lemma 10.10 into a bound on
the number of apices that a single vertex of `W` can see — the "codegree → budget" bookkeeping.
It is proved by double counting and Cauchy–Schwarz.
...
theorem card_le_of_codegree [DecidableEq ι] (W : Finset V) (T : Finset ι) (N : ι → Finset V) (m c : ℝ)
  ...
  have h1 : S1 ^ 2 ≤ n * S1 + n * (t * t) * c := by nlinarith only [hS2le, hCS, hn0]
  have h2 : n * (t * t) * c * m ≤ n * t * c * S1 := by
    have hfac : (0:ℝ) ≤ n * t * c := by positivity
    linarith only [mul_le_mul_of_nonneg_left hS1ge hfac]
