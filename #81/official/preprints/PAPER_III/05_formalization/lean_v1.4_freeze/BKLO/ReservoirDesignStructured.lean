/-
# The grid design, exported **with its structure**.

`BKLO.exists_reservoir_sparse_apexAbundant` (`BKLO/ReservoirDesign.lean`) builds the reservoir of
§10 and exports only its three numerical properties: sparsity, apex abundance, and sparsity at the
scale of `W'`.  The construction itself, however, is a *grid design*, and its structure is what any
proof of the remaining residual `BKLO.ReservoirPairingResidual4` has to use: `h²` disjoint balanced
classes inside `W'`, a grid label `(x u, y u)` for each outer vertex with small row and column
fibres, and the reservoir link of `u` equal to the `F`-neighbours of `u` inside the classes of its
row and of its column.

This file repeats the construction and exports that structure as well, as
`BKLO.IsGridReservoir`.  Nothing in `BKLO/ReservoirDesign.lean` is changed; the statement here is
strictly stronger than the one there.
...
/-! ### The structure of the grid design -/

/-- The common size of the classes of the grid design: `t ≈ |W'|/(10h²)`. -/
noncomputable def gridClassSize (ε : ℝ) (K : ℕ) (m : ℕ) : ℕ :=
  m / (10 * gridSize ε K * gridSize ε K)

variable {V : Type*} [DecidableEq V]

/-- The part of `W'` reserved for an outer vertex of cell `(p, q)`: the classes of its row
together with the classes of its column. -/
def gridRegion (h : ℕ) (C : ℕ → Finset V) (p q : ℕ) : Finset V :=
  ((Finset.range h).biUnion (fun j => C (p * h + j))) ∪
    ((Finset.range h).biUnion (fun i => C (i * h + q)))

/-- **A grid reservoir.**  `R` is the reservoir of the grid design attached to the class family `C`
and the labelling `(x, y)`: the classes are disjoint, of the common size `t`, balanced for every
vertex of `W`, the labels have small row and column fibres, and the reservoir link of an outer
vertex is exactly its `F`-link inside its own row and column. -/
structure IsGridReservoir (ε : ℝ) (K : ℕ) (W W' : Finset V) (F R : Finset (Sym2 V))
    (C : ℕ → Finset V) (x y : V → ℕ) : Prop where
  /-- the classes lie in `W'`. -/
  classSubset : ∀ i < gridSize ε K * gridSize ε K, C i ⊆ W'
  /-- the classes have the common size `t`. -/
  classCard : ∀ i < gridSize ε K * gridSize ε K,
...
/-! ### The construction, with its structure -/
...
labelling, and the identity between the reservoir links and the designed regions. -/
theorem exists_reservoir_structured
    ...
    ∃ (R : Finset (Sym2 V)) (C : ℕ → Finset V) (x y : V → ℕ),
      ...
      IsGridReservoir ε K W W' F R C x y := by
  classical
  ...
  · -- ### the grid structure of the design
    refine ⟨hCP, hCcard, hCdisj, ?_, hx, hy, hxfib, hyfib, ?_, ht1, ?_, ?_⟩
