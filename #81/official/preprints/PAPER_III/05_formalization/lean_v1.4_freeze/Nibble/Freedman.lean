...
def HasCondSubgammaMGF (X : Ω → ℝ) (V c : ℝ≥0)
  ...
  ProbabilityTheory.Kernel.HasSubgammaMGF X V c (condExpKernel μ m) (μ.trim hm)
...
lemma integrable_exp_mul (h : HasCondSubgammaMGF m hm X V c μ) (t : ℝ) :
  ...
  condExpKernel_comp_trim (μ := μ) hm ▸
    ProbabilityTheory.Kernel.HasSubgammaMGF.integrable_exp_mul h t
...
lemma ae_condExp_le (h : HasCondSubgammaMGF m hm X V c μ) {t : ℝ}
  ...
  have hb := ProbabilityTheory.Kernel.HasSubgammaMGF.mgf_le h
