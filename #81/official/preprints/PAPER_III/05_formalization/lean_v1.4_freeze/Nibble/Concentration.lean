/-
# Nibble — Module B1 : Chernoff / Hoeffding upper-tail concentration

Standalone, Mathlib-only. The reusable concentration prerequisite for the Rödl-nibble project.
-/
...
/-- **B1 — Hoeffding upper tail.** For finitely many independent random variables `X i` on a
probability space, each valued in `[0,1]`, the sum exceeds its mean by `t ≥ 0` with probability
at most `exp (-2 t² / n)`.

The proof centers each summand, applies Mathlib's Hoeffding lemma to obtain sub-Gaussian parameter
`1/4`, combines these bounds using independence, and then applies the optimized Chernoff bound for
the sum. Measurability and boundedness already imply the integrability needed for the expectations. -/
