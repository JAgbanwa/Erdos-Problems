...
* **A2 — Corollary 12.2** (§12.3), formalizable part: the *bound* `cp(G) ≤ n²/6 + (3/2)·n` in the
...
in particular to the extremal complete-split family.  The AX1/AX2 inputs are explicit, matching
the axiom-clean final assembly surface. -/
theorem Corollary_10_4b_of_AX1_AX2 (hAX1 : AX1Assumption) (hAX2 : AX2Assumption) :
    ∃ C : ℝ, ∀ p : ℕ,
      ((completeSplit p).cp : ℝ)
        ≤ ((completeSplit p).n : ℝ) ^ 2 / 6 + C * ((completeSplit p).n : ℝ) := by
  obtain ⟨C, hC⟩ := Corollary_1_2_of_AX1_AX2 hAX1 hAX2
...
number satisfies `cp(G) ≤ n²/6 + (3/2)·n` with the explicit constant of Proposition 10.1.
