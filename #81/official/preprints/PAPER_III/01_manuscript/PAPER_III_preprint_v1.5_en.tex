...
\textbf{Keywords:} clique partition; split graph; triangle packing; fractional packing; factorization; polarization; list edge coloring; graph decomposition.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={The complete-split extremizer K\_p\textbackslash vee\textbackslash overline K\_\{2p\}. The drawing is schematic: the left side represents the clique K\_p, the right side represents an independent set of order 2p, and every cross edge is present. One triangle is highlighted to show that every triangle contains a clique edge.}]{figures/fig1_complete_split_extremizer_en.png}}
\caption{The complete-split extremizer \(K_p\vee\overline K_{2p}\). The drawing is schematic: the left side represents the clique \(K_p\), the right side represents an independent set of order \(2p\), and every cross edge is present. One triangle is highlighted to show that every triangle contains a clique edge.}
\end{figure}

\textbf{Figure 1.} The complete-split graph \(K_p\vee\overline K_{2p}\). Every edge between the two parts is present, and the independent side has no internal edges. Since every triangle contains a clique edge, an edge-disjoint triangle packing uses at most \(\binom p2\) triangles; see Section 10.2. The figure is illustrative and is not used as a premise of the proof.

\subsection{1.3 Why the mesoscopic corridor appears}\label{why-the-mesoscopic-corridor-appears}
...
\item
  \textbf{Bulk:} \(\alpha\) stays away from both \(0\) and \(2\). An exact common-profile LP and a fractional cloning argument produce quadratic fractional slack. Haxell--Rödl converts the fractional packing into an integral one with only \(o(n^2)\) loss.
\item
...
\textbf{Table 1. The three proof regimes.} The bulk and sparse regimes retain their literature provenance, while the near-extremal corridor is elementary and effective.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={A schematic map of the three proof regimes on the parameter axis \textbackslash alpha=q/p: the sparse regime near 0, the bulk away from both endpoints, and the near-extremal corridor near 2.}]{figures/fig2_alpha_regimes_en.png}}
\caption{A schematic map of the three proof regimes on the parameter axis \(\alpha=q/p\): the sparse regime near \(0\), the bulk away from both endpoints, and the near-extremal corridor near \(2\).}
\end{figure}

\textbf{Figure 2.} The three parameter regimes used in the proof. The principal corridor thresholds are shown; the complete hypotheses are stated in Sections 4, 8, and 9. The endpoint regimes are asymptotic rather than hard subintervals of \([0,2]\).
...
\subsection{2.4 Asymptotic theorems used in the proof}\label{asymptotic-theorems-used-in-the-proof}
...
\subsubsection{Theorem 2.1 --- Haxell--Rödl/Yuster {[}11,17{]}}\label{theorem-2.1-haxellruxf6dlyuster-1117}
...
\subsubsection{Theorem 2.2 --- List edge coloring (proved in Appendix D)}\label{theorem-2.2-list-edge-coloring-proved-in-appendix-d}
...
\paragraph{\texorpdfstring{Remark 2.3\(^{\prime}\) --- Necessity of a density hypothesis}{Remark 2.3\^{}\{\textbackslash prime\} --- Necessity of a density hypothesis}}\label{remark-2.3prime-necessity-of-a-density-hypothesis}
...
\textbf{Lean certificates.} \texttt{PaperIII.ax2\_divisibility\_degree\_insufficient}, \texttt{PaperIII.ax2\_density\_necessary\_K7\_minus\_two\_triangles}, re-exported through \texttt{PaperIII.Obstructions}.

\subsection{2.5 Formalized auxiliary interfaces}\label{formalized-auxiliary-interfaces}
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Uniform} & \((1/3,1/3)\) & \(\bigl(\binom p2+qd\bigr)/3\) & All relevant edge orbits are covered at the uniform fractional rate. \\
\textbf{Separated} & \((1,1)\) & \(\binom d2+\binom r2\) & The two clique blocks \(N\) and \(R\) are paid internally, with no weight on the crossing orbit. \\
\textbf{Hot neighborhood} & \((1,1/3)\) & \(\binom d2+\bigl(dr+\binom r2\bigr)/3\) & The common neighborhood \(N\) is paid in full, while the residual clique structure remains fractional. \\
\end{longtable}
}
...
\section{4. Fractional Cloning and the Exact Fractional Margin}\label{fractional-cloning-and-the-exact-fractional-margin}
...
\subsubsection{Lemma 4.1 --- Fractional cloning bound}\label{lemma-4.1-fractional-cloning-bound}
...
\subsubsection{Corollary 10.2a --- Exact complete-split benchmark}\label{corollary-10.2a-exact-complete-split-benchmark}
...
The Lean development records the upper bound, lower bound, and equality as \texttt{Byproduct\_completeSplit\_cp\_le\_exact\_value}, \texttt{Byproduct\_completeSplit\_cp\_ge\_exact\_value}, and \texttt{Byproduct\_completeSplit\_cp\_exact\_value}. Appendix E.5 retains only the exact normal form and the logically distinct forced-linear-coefficient statement.

\subsubsection{Corollary 10.2b --- Sharp quadratic term and forced linear coefficient}\label{corollary-10.2b-sharp-quadratic-term-and-forced-linear-coefficient}
...
\textbf{Lean certificate.} \texttt{PaperIII.Corollary\_1\_2\_sharp}, assembled from \texttt{PaperIII.Corollary\_1\_2}, \texttt{PaperIII.Byproduct\_leading\_constant\_forced}, and \texttt{PaperIII.Byproduct\_completeSplit\_cp\_sharp}. The declaration \texttt{Byproduct\_leading\_constant\_forced} certifies the assertion about the linear coefficient; sharpness of the quadratic coefficient follows from the exact complete-split value as \(p\to\infty\).

\subsection{10.3 Retrospective on the corridor decomposition}\label{retrospective-on-the-corridor-decomposition}
...
\subsubsection{Corollary 10.4a --- Exact common-neighborhood triangle packing}\label{corollary-10.4a-exact-common-neighborhood-triangle-packing}

For all integers \(p\ge3\), \(q\ge1\) and \(0\le d\le p\), the graph \(H(p,q,d)\) (clique of order \(p\); \(q\) independent vertices, each with the same neighborhood of size \(d\)) satisfies \[
...
\subsubsection{Corollary 10.4b --- Threshold graphs}\label{corollary-10.4b-threshold-graphs}

Since \(K_p\vee\overline K_{2p}\) is a threshold graph and every threshold graph is split, Theorem 1.1 gives, in particular, the linear-error bound \(\operatorname{cp}(G)\le n^2/6+Cn\) for all threshold graphs, with the same sharp leading constant witnessed inside that subclass.
...
The following corollaries restate results from Sections 5 and 7 directly in packing language. They are formalized as \texttt{factorization\_assignment\_packing}, \texttt{double\_factorization\_packing}, and \texttt{reserved\_gain\_packing\_bound\_subset}. All three are unconditional: none uses Theorem 2.1 or Theorem 2.3.

\textbf{Corollary 5.1a (max-over-assignments).} For a factorization \(E(K_p)=F_1\sqcup\cdots\sqcup F_{r_p}\) and \(q\ge r_p\), \[
...
\textbf{Proposition 7.4 (robust shifted-center packing bound).} Let \(R\subseteq K\), \(Q=K\setminus R\), \(\rho=|R|\), \(b=|Q|\); for \(i\) write \(g_i=|N_i\cap R|\), \(h_i=|N_i\cap Q|\), \(t_i=b-h_i\). Choose \(J\subseteq I\) with \(q_J=|J|\ge r_b=\chi'(K_b)\), \(b\ge\chi'(K_\rho)\), and \(h_i\ge\max\{\rho,\,q_J-r_b\}\) for every \(i\in J\). Then \[
...
\textbf{Scope.} These are exact packing statements. Theorem 4.2 is not used in an exact inequality: its role is to create quadratic slack in the bulk, not to provide an exact bound. Likewise, a \(\tau_3\le2\nu_3\) statement of Tuza type for split graphs is not a corollary of this paper and is treated separately.

\subsubsection{Proposition 10.5 --- Explicit corridor bounds}\label{proposition-10.5-explicit-corridor-bounds}
...
\textbf{Remark (cloning terminology).} Lemma 4.1 and the clone-symmetrization step of Paper II use the same underlying replication operation in different roles. Paper II uses pairwise cloning to move a chordal graph toward a complete-split terminal graph. Lemma 4.1 uses \(q\)-fold profile replication on the cover side to compare an arbitrary profile with a common-profile graph. The established term \emph{fractional cloning} is retained for the present inequality. Paper IV uses the term \emph{clone step} for a different operation, the vertex-copy substitution that replaces one neighborhood rather than replicating a profile.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Stability of the fractional extremal theorem.} Upgrade the exact chordal fractional theorem of Paper II to a stability statement: every chordal graph whose value is within \(\delta n^2\) of the fractional maximum is within \(\varepsilon n^2\) edge edits of a maximizing complete split graph. Away from near-extremality, the quadratic fractional slack absorbs the general integrality loss exactly as in Section 4.
\item
  \textbf{Robust corridor mechanisms.} Extend the factorization, polarization, and shifted-center arguments of Sections 5--7 from split graphs to \(\varepsilon n^2\)-perturbations of complete split graphs, so that they apply to the near-extremal graphs produced by stability.
\item
  \textbf{Clique-tree assembly.} Patch the local estimates across the clique tree by an edge-ownership ledger that preserves separator capacities, in the spirit of shifted-center completion, so that no edge is charged twice.
\end{enumerate}
...
\subsection{11.6 Formal-verification perimeter and current status}\label{formal-verification-perimeter-and-current-status}

The formalization discussion is confined to this section and the release record in Section 13. The current formal snapshot separates the unconditional final assembly from its downstream interfaces. The public aggregate root \texttt{PaperIII} imports both \texttt{PaperIII.Theorem\_1\_1\_Final} and \texttt{PaperIII.PublicAPI}. The final module discharges the two interfaces used by the paper and exports Theorem 1.1 and Corollary 1.2. \texttt{PaperIII.CanonicalTrianglePacking} makes \texttt{PaperIII.nu3} and \texttt{PaperIII.nu3Star} the manuscript-facing quantities and proves their equality with the edge-hypergraph implementations used by AX1; it also proves \texttt{PaperIII.tau3Star\ =\ PaperIII.nu3Star} and identifies the cover-side AX1 interface with the packing-side statement in Section 2. \texttt{PaperIII.PaperImprovements} contains the paper-facing certificates; \texttt{PaperIII.PublicAPI}, \texttt{PaperIII.OfPartition}, and \texttt{PaperIII.Obstructions} expose downstream interfaces. The directed canonical gate reports the following axiom footprint for these theorem surfaces:
...
\textbf{Table 2. Formalization surfaces.} Statuses in this table are records of the sealed local freeze; they are not a substitute for independent reproduction.
...
\bottomrule\noalign{}
\endlastfoot
\texttt{PaperIII} & aggregate/compatibility root & public-root build completed 8,455 jobs; exit 0; imports the final theorem and public API \\
\texttt{PaperIII.MainNibble} & theorem and corollary assembly through explicit AX1/AX2 interfaces & compiled in the recorded closure \\
\texttt{PaperIII.Theorem\_1\_1\_Final} & unconditional discharge of AX1/AX2 and final theorem surfaces & compiled; final theorem footprint is foundational-only \\
\texttt{PaperIII.CanonicalTrianglePacking} & canonical \texttt{nu3}, \texttt{nu3Star}, \texttt{tau3Star} and AX1 statement bridges & compiled; exact target-side bridges recorded \\
\texttt{PaperIII.CanonicalTrianglePacking\allowbreak{}Gate} & statement and axiom gate for the canonical interfaces & exit 0; foundational-only footprints recorded \\
\texttt{PaperIII.PaperImprovements} and \texttt{PaperIII.PaperImprovementsGate} & paper-facing corollaries and selected auxiliary certificates & compiled in the consolidated gate run \\
\texttt{PaperIII.PublicAPI}, \texttt{PaperIII.OfPartition}, \texttt{PaperIII.Obstructions} & downstream API, split-partition transport, and obstruction certificates & compiled in the recorded closure \\
\texttt{FREEZE\_REPORT.md} and \texttt{ESCAPE\_HATCH\_ASSESSMENT.md} & immutable build, axiom and raw-token assessment & generated and included in the sealed formal archive \\
\end{longtable}
}

At the \texttt{SimpleGraph} level, \texttt{Theorem\_1\_1\_of\_splitPartition\_uncond} accepts a finite graph equipped with a clique/independent partition. The construction \texttt{SplitGraph.ofPartition} is connected to the original graph by \texttt{ofPartitionIso}; the vertex count, edge count, \(\nu_3\), and \(\Phi\) are transported through graph-isomorphism invariance, including \texttt{nu3\_congr\_of\_iso}. Other type-specialized theorem forms remain in \texttt{PaperIII.PublicAPI} for downstream use but are not mathematical results of the paper and are not catalogued here.
...
\subsection{11.7 Reusable formalization components}\label{reusable-formalization-components}

The formalization also produced a standalone \texttt{Contrib.*} library whose modules import only Mathlib. Four groups are especially relevant beyond this paper:

\begin{itemize}
\tightlist
\item
  finite packing/covering LP strong duality via a conic-Farkas argument (\texttt{Contrib.LPStrongDuality});
\item
  variance-sensitive concentration and martingale tails (\texttt{Contrib.BennettBernstein}, \texttt{Contrib.HoeffdingUpper}, \texttt{Contrib.ConditionalHoeffding}, and \texttt{Contrib.FreedmanBernstein});
\item
  Dirac-type Hamiltonicity and matching results (\texttt{Contrib.DiracHamiltonian} and \texttt{Contrib.SpreadMatchingDirac});
\item
  finite max-flow/min-cut and selected design-theoretic components (\texttt{Contrib.MaxFlowMinCut} and \texttt{Contrib.FanoSTS}).
\end{itemize}
...
\subsubsection{Algorithmic remark 12.3 --- Effective corridor construction}\label{algorithmic-remark-12.3-effective-corridor-construction}

In the near-extremal corridor covered by Proposition 10.5 (explicit \(p_0,s_0\)), the proof is constructive at the level of its mathematical ingredients: it uses one- and double-factorizations of \(K_p\), reserved-vertex selection, list edge coloring, and matching extraction. With explicit representations of these objects and standard implementations of the underlying routines, the construction is expected to run in polynomial time and to output a clique partition of size \(n^2/6+3n/2\). The manuscript does not yet fix an input model, data structures, or a complete implementation-level complexity proof, so no standalone polynomial-time theorem is claimed here. Outside the corridor, the guarantee also relies on the non-constructive bulk input (Theorem 2.1).
...
Two pieces of evidence support a Simonovits-type stability statement. First, the supplied audit report records an exhaustive enumeration at \(n=9\) (all split graphs with \(|K|=3\)). In that computation, graphs within an additive constant of the maximum lie within bounded edit distance of the complete-split family, while genuinely dispersed profiles fall a linear amount below the maximum. Second, the fractional margin of Theorem 4.2 already yields a level-set bound: away from near-extremality the quadratic slack \(\mu(\alpha)p^2\) is strictly positive, so any near-maximizer must have \(\alpha\) near the extremal ratio and small profile dispersion. A full stability theorem would still require quantifying the gain of a single symmetrization step and controlling the directions along which \(\Phi^*\) is flat; we leave it as an open problem, most naturally phrased in the cut (graphon) metric to absorb the choice of nearest complete-split graph.
...
The mathematical source is this manuscript. The manuscript-facing formal surfaces are \texttt{PaperIII.Theorem\_1\_1\_Final}, \texttt{PaperIII.CanonicalTrianglePacking}, \texttt{PaperIII.PublicAPI}, \texttt{PaperIII.OfPartition}, and \texttt{PaperIII.Obstructions}. \texttt{PaperIII.PaperImprovementsGate} is the byproduct gate, and \texttt{PaperIII.CanonicalTrianglePacking\allowbreak{}Gate} checks the exact correspondence between the canonical packing parameters and the Nibble/Yuster model used to discharge AX1.

The formal development is sealed as \texttt{PAPER\_III\_lean\_v1.4\_freeze.zip}, with SHA-256 \texttt{79ee24c38fd776bc2585a0c3c996e30817f0829fc5064463bdbde0fa2d3d7104}. It contains 704 Lean source files and the three pinned Lake/toolchain files listed in the 707-entry source manifest. The recorded verification uses Lean 4.28.0 and Mathlib v4.28.0 at commit \texttt{8f9d9cff6bd728b17a24e163c9402775d9e6a365}.
...
\bottomrule\noalign{}
\endlastfoot
public aggregate-root build & \texttt{gate\_logs/run\_2026-08-22\_CCS\_NOTEBOOK456\_resumed/logs/02\_build\_public\_root\_clean.log} & exit 0; 8,455 jobs completed; clean-origin process resumed after an application restart \\
query-root completion & \texttt{gate\_logs/run\_2026-08-22\_CCS\_NOTEBOOK456\_resumed/logs/03\_build\_query\_roots\_incremental.log} & exit 0; 8,444 jobs completed \\
canonical statement-and-axiom gate & \texttt{PaperIII.CanonicalTrianglePacking\allowbreak{}Gate} & exit 0; target-side integral and fractional bridges checked \\
complete theorem-level axiom record & eight \texttt{FreezeAxioms*.lean} files & exit 0; only \texttt{propext}, \texttt{Classical.choice}, and \texttt{Quot.sound} \\
quantitative Lean-closure regression & \texttt{FreezeAxiomsAuditClosure.lean} and \texttt{FreezeAxiomsAX1Closure.lean} & all named closure surfaces present with foundational-only footprints \\
escape-hatch assessment & raw-token and focused build/footprint checks & no active \texttt{sorry} warning, no active \texttt{native\_decide}, and no project axiom in a canonical surface \\
...
\textbf{Review boundary:} the independent reproduction and external adversarial audit are complete, and the novelty review is corpus-bounded. Human peer review and specialist priority determination have not been performed. The public tag and commit are repository metadata rather than premises of the theorem.
...
\section{Appendix D. Self-Contained List Edge Coloring}\label{appendix-d.-self-contained-list-edge-coloring}
...
A \textbf{kernel} of a digraph \(D\) is an independent set \(K\) of vertices such that every vertex outside \(K\) has an out-neighbor in \(K\). \(D\) is \textbf{kernel-perfect} if every induced subdigraph has a kernel. A kernel of a nonempty digraph is nonempty.

\subsubsection{Lemma D.1 --- Kernel coloring lemma}\label{lemma-d.1-kernel-coloring-lemma}
...
Let \(B\) be a bipartite graph with parts \(U\) and \(R\) in which every vertex \(z\) has a linear preference order \(>_z\) on its incident edges. A matching \(M\subseteq E(B)\) is \textbf{stable} if every edge \(f\notin M\) has an endpoint \(z\) that is covered by an edge \(e\in M\) with \(e>_zf\).

\subsubsection{Lemma D.2 --- Gale--Shapley {[}9{]}}\label{lemma-d.2-galeshapley-9}
...
\subsubsection{Theorem D.3 --- Galvin, maximum-degree case}\label{theorem-d.3-galvin-maximum-degree-case}
...
\textbf{Step 4: conclusion.} Every edge has \(|L(e)|\ge\Delta\ge d^+_D(e)+1\), so Lemma D.1 yields a proper coloring of the underlying graph of \(D\) from the lists --- that is, a proper list edge coloring of \(B\). \(\square\)
...
\section{Appendix E. Selected Formalization-Derived Corollaries}\label{appendix-e.-selected-formalization-derived-corollaries}
...
The theorem names follow \texttt{PaperIII.PaperImprovements} and the standalone \texttt{Contrib.*} package. Their release status is governed by the consolidated build and axiom gate in Section 13.
...
\subsection{E.1 Factorization packing bounds}\label{e.1-factorization-packing-bounds}

\subsubsection{Corollary E.1.1 --- Single factor-assignment packing}\label{corollary-e.1.1-single-factor-assignment-packing}
...
\textbf{Lean certificate.} \texttt{Byproduct\_factorization\_assignment\_packing}.

\subsubsection{Corollary E.1.2 --- Double-factorization lower bound}\label{corollary-e.1.2-double-factorization-lower-bound}
...
\textbf{Lean certificate.} \texttt{Byproduct\_double\_factorization\_packing}.

\subsubsection{Corollary E.1.3 --- Reserved-gain packing bound}\label{corollary-e.1.3-reserved-gain-packing-bound}
...
\textbf{Lean certificate.} \texttt{Byproduct\_reserved\_gain\_packing\_bound\_subset}.

\subsection{E.2 Effective corridor interfaces}\label{e.2-effective-corridor-interfaces}

\subsubsection{\texorpdfstring{Corollary E.2.1 --- Short-corridor constant \(3/2\)}{Corollary E.2.1 --- Short-corridor constant 3/2}}\label{corollary-e.2.1-short-corridor-constant-32}
...
\textbf{Lean certificate.} \texttt{Byproduct\_short\_corridor\_constant\_three\_halves}.

\subsubsection{Corollary E.2.2 --- Packing-to-partition bridge}\label{corollary-e.2.2-packing-to-partition-bridge}
...
\textbf{Lean certificates.} \texttt{Byproduct\_cp\_le\_Phi}, \texttt{Byproduct\_low\_corridor\_cp\_three\_halves}, \texttt{Byproduct\_effective\_low\_corridor\_cp\_bound}.

The mid-corridor and high-ratio specializations are already stated in Proposition 10.5 and are not repeated here.

\subsection{\texorpdfstring{E.3 Exact common-profile family \(H(p,q,d)\)}{E.3 Exact common-profile family H(p,q,d)}}\label{e.3-exact-common-profile-family-hpqd}
...
\subsubsection{Corollary E.3.1 --- Exact fractional optimum}\label{corollary-e.3.1-exact-fractional-optimum}
...
\textbf{Lean certificate.} \texttt{Byproduct\_commonProfile\_tau3Star\_eq\_F}.

\subsubsection{Corollary E.3.2 --- Phase regions}\label{corollary-e.3.2-phase-regions}
...
\textbf{Lean certificates.} \texttt{Byproduct\_commonProfile\_tau3Star\_eq\_uniform\_region}, \texttt{Byproduct\_commonProfile\_tau3Star\_eq\_separated\_region}, \texttt{Byproduct\_commonProfile\_tau3Star\_eq\_hot\_region}.

\subsection{E.4 Unified fractional margin}\label{e.4-unified-fractional-margin}

\subsubsection{Corollary E.4.1 --- Fractional slack lower bound}\label{corollary-e.4.1-fractional-slack-lower-bound}
...
\textbf{Lean certificate.} \texttt{Byproduct\_unified\_fractional\_margin}.

\subsection{E.5 Complete-split sharpness package}\label{e.5-complete-split-sharpness-package}

Let \(S_p=K_p\vee\overline K_{2p}\), so \(|V(S_p)|=3p\).

\subsubsection{Corollary E.5.1 --- Exact value and normal form}\label{corollary-e.5.1-exact-value-and-normal-form}
...
\textbf{Lean certificates.} \texttt{Byproduct\_completeSplit\_cp\_exact\_value}, \texttt{Byproduct\_completeSplit\_cp\_sharp}.

\subsubsection{Corollary E.5.2 --- Forced linear coefficient}\label{corollary-e.5.2-forced-linear-coefficient}
...
\textbf{Lean certificates.} \texttt{Byproduct\_leading\_constant\_forced}, packaged with the upper bound and exact family as \texttt{Corollary\_1\_2\_sharp}.

The weaker certificate \texttt{Byproduct\_leading\_constant\_sharp}, which follows immediately from the exact normal form, is not separately catalogued.

\subsection{E.6 Reusable formalized components}\label{e.6-reusable-formalized-components}
...
\subsubsection{E.6.1 Finite packing/covering LP duality}\label{e.6.1-finite-packingcovering-lp-duality}
...
\textbf{Lean certificates.} \texttt{Byproduct\_triangle\_LP\_duality}; standalone form \texttt{Contrib.LPDuality.lp\_strong\_duality} in \texttt{Contrib.LPStrongDuality}.

\subsubsection{E.6.2 Dirac-type matching and dense correction}\label{e.6.2-dirac-type-matching-and-dense-correction}
...
\textbf{Lean certificates.} \texttt{Byproduct\_dirac\_perfect\_matching\_even}, \texttt{Byproduct\_dirac\_near\_perfect\_matching}, \texttt{Byproduct\_dense\_divisible\_correction\_edges}; standalone matching form in \texttt{Contrib.SpreadMatchingDirac}.

\subsubsection{E.6.3 Uniform nibble interface}\label{e.6.3-uniform-nibble-interface}

Assume the formalized majority-nibble interface \texttt{Nibble.NibbleTheoremMost}. Let \(r\ge2\). For every \(\beta>0\), it supplies parameters \(\mu,\eta,d_0>0\) such that every \(r\)-uniform hypergraph satisfying the majority near-regularity and codegree hypotheses at scale \(d\ge d_0\) has a matching \(M\) with

\[
|M|\ge(1-\beta)\frac{|W|}{r}.
\]

\textbf{Lean certificate.} \texttt{Byproduct\_almostPerfectMatching\_uniform}.
...
\subsection{E.7 Packing witnesses and the edge-gap form}\label{e.7-packing-witnesses-and-the-edge-gap-form}

\subsubsection{Corollary E.7.1 --- Attainment and fractional edge bound}\label{corollary-e.7.1-attainment-and-fractional-edge-bound}
...
\textbf{Lean certificates.} \texttt{Byproduct\_nu3\_achieved}, \texttt{Byproduct\_nu3star\_le\_edges\_div\_three}.

\subsubsection{Corollary E.7.2 --- Majority-nibble edge-gap form}\label{corollary-e.7.2-majority-nibble-edge-gap-form}

For every \(\beta>0\), under the majority near-regularity and codegree hypotheses supplied by \texttt{NibbleTheoremMost},
...
\textbf{Lean certificate.} \texttt{Byproduct\_nu3star\_sub\_nu3\_le\_edge\_gap\_most}.
