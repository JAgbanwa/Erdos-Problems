...
def nu3_ILP(tris, edges):
    ...
    # verify the solver actually reached a proven optimum (adversarial-audit fix):
    if pulp.LpStatus[status] != "Optimal":
        raise RuntimeError(f"CBC did not prove optimality: status={pulp.LpStatus[status]}")
    val = pulp.value(prob.objective)
    if val is None:
        raise RuntimeError("CBC returned no objective value")
