#!/usr/bin/env python3
"""Paper III v1.3 external adversarial audit -- gate E0, intake, provenance and sealing.
...
Checks, in order:
  ...
  7  that every label refers to v1.3 and not a stale earlier version
  8  the auditor's own SHA-256 manifest of the received target

The build workspace 05_formalization/lean_v1.3_candidate/ is EXCLUDED: the request states it
...
T = "C:/p3v13"
OUT = "C:/erdos_audit/v13/E0"
EXCLUDE_DIRS = ("05_formalization/lean_v1.3_candidate",)
DECLARED_FREEZE = "2eb0ff20a9dae6610a46026355374570d5afdfea89837ea7f9dd29da10b9d300"
...
def zip_checks(zp):
    ...
    with zipfile.ZipFile(zp) as z:
        ...
        # v1.3 labelling inside the archive
        res["mentions_v1_2"] = [n for n in names if "v1.2" in n or "v1_2" in n]
        res["mentions_v1_3"] = sum(1 for n in names if "v1.3" in n or "v1_3" in n)
    return res


def stale_label_scan(files):
    """Every textual artifact must refer to v1.3, not a stale earlier label."""
    hits = []
...
def main():
    ...
    res = {"gate": "E0", "paper": "PAPER_III", "target": "preprint_draft_v1.3",
           "request_sha256":
               "a0569fa455d70ee3e8af8ddb4362c52863de69d830bc76975bb6f9d22d9e12c0",
           "excluded_from_target": list(EXCLUDE_DIRS),
           "exclusion_basis": ("the audit request states lean_v1.3_candidate is a local "
    ...
    res["manuscript_sidecar"] = verify_sidecar(
        "01_manuscript/PAPER_III_preprint_draft_v1.3_SHA256.txt",
        [os.path.join(T, "01_manuscript"), T])

    # 3 the freeze archive sidecar
    fz = "05_formalization/lean_v1.3_freeze"
    res["freeze_zip_sidecar"] = verify_sidecar(
        f"{fz}/PAPER_III_lean_v1.3_freeze.zip.sha256",
        [os.path.join(T, fz), T])
    ...
    res["freeze_zip"] = zip_checks(os.path.join(T, fz,
                                                "PAPER_III_lean_v1.3_freeze.zip"))
    ...
    print(f"  entradas etiquetadas v1.3: {z['mentions_v1_3']}")
